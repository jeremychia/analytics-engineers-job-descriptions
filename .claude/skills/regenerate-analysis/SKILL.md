...
5. **Updates `index.html` automatically** for everything sourced from `data.json` (no code changes needed — dashboard auto-fetches it); the "Responsibility language" relationship panels additionally auto-fetch `responsibility_classification.json` and recompute live, but only if step 3 ran first
...
analysis/full-analysis.html (auto-fetches)    analysis/responsibility_taxonomy.py
           ↓                              ↓ (reads data/*/jd_archive.md + data.json)
Interactive browser dashboard ✨      responsibility_classification.json
                                           ↓
                              index.html "Responsibility language" panels
                              (auto-fetched separately, client-side merge —
                               see RT_THEME_FIELDS in index.html)
```
...
7. Commit all files with message like:
   ```
...
- `analysis/full-analysis.html` — no edit needed for `data.json`-sourced content (auto-fetches); the "Responsibility language" relationship panels additionally auto-fetch `responsibility_classification.json` and merge client-side (`RT_THEME_FIELDS`) — edit `full-analysis.html` directly only if adding/removing which themes get a dashboard panel, not to update numbers
