...
analysis/data.json (single compiled source)
           ↓                              ↘
analysis/index.html (auto-fetches)    analysis/responsibility_taxonomy.py
...
- `analysis/index.html` — no edit needed for `data.json`-sourced content (auto-fetches); the "Responsibility language" relationship panels additionally auto-fetch `responsibility_classification.json` and merge client-side (`RT_THEME_FIELDS`) — edit `index.html` directly only if adding/removing which themes get a dashboard panel, not to update numbers
