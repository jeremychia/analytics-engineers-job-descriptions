...
- **Cloudflare-protected pages** (curl returns a JS challenge/interstitial instead of content, common on some corporate career sites e.g. `careers.intuitive.com`): plain curl cannot get past the challenge. This is a genuine "could not verify by the tools available" case, not a signal to fall back to WebFetch — note the gate explicitly and move on; don't fabricate or silently accept a WebFetch paraphrase as if it were verified. **First, check whether the gated page is only a front for an ATS you can reach directly.** A corporate careers domain often proxies Workday, Greenhouse or SmartRecruiters, and the underlying ATS host is usually not gated. Read `robots.txt` and `sitemap.xml` (both commonly served even when the challenge blocks HTML), and look at the shape of the job id in the URL — a `jr######` slug is a Workday requisition id. Then go at the ATS API directly. Worked case: `careers.richemont.com/en/jobs/jr131943/...` returns 403 with `cf-mitigated: challenge` on every header set, including a full browser fingerprint and as Googlebot — but it is Workday underneath, and `https://richemont.wd3.myworkdayjobs.com/wday/cxs/richemont/Richemont/` serves both a POST `/jobs` search and a GET `/job/{location}/{title}_{reqId}` returning the full `jobPostingInfo.jobDescription`.
