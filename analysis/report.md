...
**Honest scope limitations:** 389 JDs (analytical cohort) is a moderate-scale dataset with tighter confidence intervals than earlier snapshots. The confidence interval on a single proportion is approximately ±4.5pp at 95% (Wilson interval) — tight enough that core dimensions (rigour, domain_risk, maturity) show directional consistency, but still wide enough that individual percentages should be read as directional signals, not precise market measurements. The geographic concentration is still primarily European/Berlin, but the APAC stratum (n=50) remains large enough to test directly against the European majority rather than merely disclaim — see §9.5 for what that comparison shows and its own, tighter limits. Generalisation to North America remains untested. These limitations are stated once here and apply to every finding in this document; they are not repeated at every mention. Mid-corpus expansions (July 13, 2026: +9 JDs; July 16, 2026: +12 JDs; July 17, 2026: +13 JDs; July 21, 2026: +21 JDs; July 22–24, 2026: +55 JDs; July 26–29, 2026: +38 JDs; July 30, 2026: +11 JDs; August 5, 2026: +15 JDs; August 6, 2026: +17 JDs to the analytical cohort, §9.9–§9.10) added new roles without statistical re-weighting, so updated findings through that point reflected raw inclusion in the analytical cohort. The 2026-07-25 dedup removed 37 duplicate records across two passes rather than adding new ones (§3, §9.6) — this tightened the corpus rather than diluting it. Every relationship in this document is re-tested at each corpus update, and significance is not treated as permanent: this revision (n=389) finds `velocity_vs_rigour × has_dbt` (§4.0's Prediction 1 comparator, χ²=3.89, p=0.14, V=0.10), `domain_risk × greenfield_vs_fix` (previously Finding B, χ²=5.67, p=0.23, V=0.09), and `stakeholder_orientation × autonomy_level` (previously Finding E, χ²=11.65, p=0.17, V=0.12) still do not clear p<0.05 — each restated below as a current null or directional read. `geo_region (APAC) × jd_authorship` (previously part of Finding G) remains below p<0.05 at this n (χ²=8.26, p=0.016, V=0.15). `data_team_maturity × work_arrangement` on the stated-arrangement subset remains significant at this n (χ²=12.89, p=0.012, V=0.16, n=240 stated).
...
**Test:** χ² for `velocity_vs_rigour` × `domain_risk` (n=389): χ²=21.53, p=0.0002, V=0.17 — stable, essentially identical to the n=372 reading (p=0.0001, V=0.18). χ² for `velocity_vs_rigour` × `has_dbt` (n=389, AE/BI only): χ²=3.89, p=0.14, V=0.10 — **still not significant.** The point estimate has held roughly flat (similar rigour-rate gap between has_dbt=False and has_dbt=True as prior snapshots), and the effect size (V=0.10) remains right at the small-effect floor used elsewhere in this document. Read this as the comparator relationship continuing to show no detectable link between rigour framing and tool adoption — not as `domain_risk` weakening; `domain_risk`'s own association with rigour is essentially unchanged. With the `has_dbt` comparator null and `domain_risk` still real, Prediction 1's core claim ("rigour is not detectably *more* tied to real risk than to vendor-adoption signal") continues to have a small, real `domain_risk` effect and no detectable `has_dbt` effect to point to: rigour framing tracks real organisational stakes, not tool adoption, at this corpus size. High-risk roles remain markedly more rigour-dominant (89% vs. 59–66% for low/moderate — §4.2), a real gradient, stable across every snapshot from n≤240 onward.
...
**Test:** `ai_role` is coded across the full analytical cohort (n=389 — see §9.3 for the pipeline-bug history). `ai_role = none` is 66% of the cohort (essentially flat against every prior snapshot) — against the dbt 2026 report's claim of 72% *daily* AI coding use. χ² for `ai_role` × `stakeholder_orientation` (n=389): χ²=10.81, p=0.21, V=0.12 — **remains not significant**, consistent with every prior reading. `ai_enabler` (79 of 389, 20%) still concentrates somewhat in `internal_data` (51%) and `mixed` (20%) stakeholder orientation versus `ai_user` (53, 14%) which spreads more toward `mixed`/`commercial` (28%/11%), and the association's weakness is stable rather than still trending toward zero. **Prediction 2's second half (non-random concentration) continues to not hold at conventional significance; the first half (large adoption-claim/hiring-criterion gap) still holds — `none` is 66% of hiring criteria against the survey's 72% daily-use claim.**

**What this buys the document:** two explicit, checkable predictions, stated before the findings that test them, with the statistical result reported honestly as it changes — including when a corpus expansion moves an earlier reading, as happened here. Prediction 1's `domain_risk` comparator has held significant at essentially the same effect size from n=272 through the current n=389; its `has_dbt` comparator has remained clearly non-significant across every recent snapshot including this one (p=0.14, V=0.10). Prediction 2 flipped a different direction earlier on: a marginal, medium-effect result at a small, biased coded subset (n=86) gave way to a clear non-result once the same three dimensions were coded across the full cohort, and that non-result has held at every subsequent n including n=389. These trajectories are instructive about statistical power and sample composition rather than embarrassing reversals to paper over — this is the fix for Appendix B's "six theories, none tested" critique — not a stronger claim than the data supports, but an honest, and honestly-updated, one.
...
| velocity_vs_rigour | n | % (analytical, n=389) |
|--------------------|---|---|
| rigour | 276 | 71% |
| mixed | 106 | 27% |
| velocity | 7 | 2% |

**71% of JDs in the analytical cohort signal a rigour orientation**, unchanged from the 71% reading at n=372 — the fourth consecutive snapshot at exactly 71%, since the most recent batch (§9.10) was not disproportionately rigour- or velocity-skewed. Pure velocity holds flat at 2% (7 JDs across 389). This remains the clearest single-dimension finding in the dataset — the percentage has drifted down across most expansions (80% → 75% → 75% → 72% → 73% → 71% → 71% → 71% → 71%) as the corpus has diversified into more low-risk, gaming/consumer, and now AI-forward roles (§3), but the band has stayed narrow (71-73%) since n=272, including the first sizeable non-European stratum. Per §4.2, rigour framing shows a small but statistically real gradient with domain risk; per §4.0, its gradient with tool adoption (`has_dbt`) continues to show no detectable effect at this n — an institutionalised norm that still responds modestly to real stakes, but not detectably to tool adoption.
...
| domain_risk | n | % (analytical, n=389) |
|-------------|---|---|
| moderate | 266 | 68% |
| high | 96 | 25% |
| low | 27 | 7% |
...
| data_team_maturity | n | % (analytical, n=389) |
|--------------------|---|---|
| mid | 234 | 60% |
| mature | 97 | 25% |
| early | 58 | 15% |

**Just under two-thirds of roles are mid-stage.** Early-stage roles hold at 15%; genuinely mature organisations are 25% — both essentially unchanged from the n=372 snapshot (16%/25%). This batch (§9.10) added zero early-stage roles, nudging the early share down fractionally. APAC's own maturity mix (§9.5) continues to track the corpus average closely.

**Maturity × greenfield_vs_fix cross-tab** (χ²=195.12, p<0.0001, V=0.50, n=389 — the strongest relationship in the dataset, and stable across every expansion and dedup pass):
...
χ²=32.41, p<0.0001, V=0.20 (n=389, essentially unchanged from n=372's V=0.21). Early-stage roles offer strategic autonomy at 64% — still far above mid- or mature-stage roles (25% and 31%). Mid-stage remains the least strategic tier despite being the largest market segment. The core pattern — greenfield work and direction-setting cluster at early-stage companies — holds.
...
| stakeholder_orientation | n | % (analytical, n=389) |
|-------------------------|---|---|
| internal_data | 196 | 50% |
| mixed | 71 | 18% |
| commercial | 50 | 13% |
| finance | 44 | 11% |
| product | 28 | 7% |

**50% of roles in this cohort primarily serve internal data consumers** — other analysts, data scientists, ML engineers, or the platform itself. This remains the dominant archetype in the market, essentially unchanged from the n=372 reading (51%) — the decline that ran through earlier snapshots has clearly stabilised in the 50-54% band. APAC's own stakeholder mix isn't a standout finding — see §9.5.

**Cross-tab with rigour** (χ²=54.12, p<0.0001, V=0.26, n=389):

| stakeholder_orientation | mixed | rigour | velocity | n |
|-------------------------|-------|--------|----------|---|
| finance | 7% | 93% | 0% | 44 |
| internal_data | 17% | 82% | 2% | 196 |
| product | 43% | 54% | 4% | 28 |
| commercial | 46% | 50% | 4% | 50 |
| mixed | 49% | 49% | 1% | 71 |

Finance and internal_data roles remain the most rigour-dominant (82–93%); commercial and product roles are close to evenly split between rigour and mixed framing. This relationship remains clearly significant at n=389 (V=0.26, unchanged from n=372) — still the clearest stakeholder-level driver of rigour/velocity framing in the dataset.
...
| autonomy_level | n | % (analytical, n=389) |
|----------------|---|---|
| mixed | 143 | 37% |
| strategic | 126 | 32% |
| execution | 120 | 31% |

The three-way split persists, and `mixed` continues to edge ahead of `strategic` (37% vs. 32%, essentially the same gap as the n=372 reading). Given that similar small gaps at earlier snapshots have been shown to sit well within the range this corpus size produces, this is not evidence of a trend toward `mixed` overtaking `strategic` — just where the current n happens to land. This even distribution reinforces that autonomy cannot be read from title or seniority label alone; context (maturity, stakeholder, domain risk) matters much more.

**Seniority × autonomy** (χ²=76.75, p<0.0001, V=0.31, n=389):
...
| jd_authorship | n | % (analytical, n=389) |
|---------------|---|---|
| hiring_manager | 276 | 71% |
| mixed | 82 | 21% |
| recruiter | 31 | 8% |

**Hiring-manager-authored JDs are 71% of the corpus**, unchanged from 71% at n=372. **The APAC gap remains below p<0.05 at this n:** APAC roles are 88% hiring_manager-authored vs. 68% for the rest of the corpus (χ²=8.26, p=0.016, V=0.15, n=389 — essentially unchanged from the n=372 reading of p=0.017; see §9.5). This dimension remains the lowest in LLM self-consistency (0.58) — a codebook-ambiguity signal — so this finding should be read as "a real, currently-significant gap," carrying that caveat, not as a newly-confirmed claim independent of it; the relationship has crossed the conventional threshold in both directions across earlier snapshots, which is itself informative about how close to the boundary it sits.

**Cross-tab with rigour** (χ²=9.14, p=0.058, V=0.11, n=389): hiring_manager 75% rigour / 23% mixed / 1% velocity; mixed 61% rigour / 37% mixed / 2% velocity; recruiter 58% rigour / 39% mixed / 3% velocity. Still not significant at conventional thresholds — unlike domain_risk (§4.2), authorship sophistication continues to show no detectable relationship with rigour framing.

**Cross-tab with has_dbt** (χ²=23.76, p<0.0001, V=0.25, n=377, AE/BI only):

| jd_authorship | has_dbt=False | has_dbt=True | n |
|---------------|---------------|---------------|---|
| hiring_manager | 27% | 73% | 268 |
| mixed | 48% | 52% | 79 |
| recruiter | 63% | 37% | 30 |

Hiring-manager-authored JDs name dbt at roughly 2× the rate of recruiter-authored ones (73% vs. 37%), unchanged from the n=372 reading — the relationship is stable and the effect size held (V=0.25). Read against Deming & Kahn's revealed-preference framework (§6): a hiring-manager-named tool requirement is a higher-fidelity signal than a recruiter-named one — the manager screens for it because they use it; the recruiter may be pulling from a template. The practical implication: dbt's *absence* in a recruiter-authored JD is weaker evidence the team doesn't use it than absence in a hiring-manager-authored JD.
...
The earlier draft's finding — mature teams have the widest named-stakeholder count — is directionally intact (2.69 vs. 2.37 vs. 2.36), essentially unchanged from the n=372 reading (2.67/2.35/2.36) — the ordering hasn't changed at any snapshot, but the gap between mature and early still hasn't reached a magnitude worth treating as reliable given this dimension's known noise (§9.1's evidence-quote pass-rate problem). **This dimension still does not currently support a confident finding.** It is retained in the codebook for future corpus growth, but no claim built on it should be treated as established.

---

### 4.8 dbt prevalence: real but not universal

`has_dbt` is a required-or-preferred tool flag, not a Layer B dimension. **66% of AE/BI roles (n=377) mention dbt.**

This is consistent with dbt's own claim that it has become the field standard, but roughly one in three AE/BI roles run on a stack without it. The prevalence has held essentially flat across the last several snapshots (68%→66%→65%→65%→66%→65%→64%→65%→65%→66%), including through both dedup passes and every subsequent expansion — dbt prevalence among APAC AE/BI roles specifically (66%, n=50) remains close to the corpus average (§9.5). This market includes a meaningful share of Databricks SQL, BigQuery-native, and Spark-first stacks. A survey distributed exclusively through dbt's community channels cannot see that portion of the market by construction — this is the self-selection constraint from §2, made concrete. The JD data documents this blind spot directly: roughly one in three roles don't name dbt at all, stable across ten consecutive corpus snapshots.
...
**Chi-squared (χ²):** applied to categorical × categorical pairs with adequate expected cell frequencies. At n=389, the minimum detectable effect (α=0.05, 80% power) for a typical cross-tab is Cramér's V ≈ 0.16 — essentially the same as the n=372 threshold. Findings below the current threshold are still directional only.
...
### Finding D: Seniority predicts autonomy moderately for the modal title, strongly at the tails (χ², p<0.0001, V=0.31, n=389)

Full cross-tab in §4.5. "Mid" (n=197) remains the largest title cohort by count, but "Senior" (n=153) remains the more informative title, spanning execution/mixed/strategic at 20/32/48 — noticeably more strategic-leaning than the corpus-wide split, consistent with prior snapshots. Staff, manager, and lead titles (n=6, n=5, n=13) predict strategic scope near-perfectly (62–100%), but the cells remain too small to generalise with confidence.

**Theoretical read — Spence (1973), signalling, now more mixed than contradicted:** if job titles were reliable, costly-to-fake signals, "Senior" should predict autonomy cleanly. At n=389 "Senior" remains a meaningfully informative signal (48% strategic vs. an overall cohort rate of 32%), consistent with prior snapshots — the signalling account continues to look less contradicted than the n=123 baseline suggested, though roughly a fifth of senior roles remain pure execution, so the signal stays noisy. Staff/manager/lead titles retain the strongest signal value, consistent with being rarer and costlier to award, but the cells are too small here to treat as confirmed.

---

### Finding E: stakeholder orientation and autonomy level — a tested null (χ², p=0.168, V=0.12, n=389)

| stakeholder_orientation | execution | mixed | strategic |
|-------------------------|-----------|-------|-----------|
| finance (n=44) | 39% | 30% | 32% |
| commercial (n=50) | 20% | 50% | 30% |
| mixed (n=71) | 27% | 41% | 32% |
| internal_data (n=196) | 34% | 35% | 31% |
| product (n=28) | 25% | 25% | 50% |

Finance-facing roles still look among the most execution-concentrated segments (39%) and product-facing roles still look the most strategic (50%), the same direction as every prior snapshot, and the test remains just short of p<0.05 at this n (p=0.168, up slightly from p=0.152 at n=372 — further from the line, still on the null side). This relationship has crossed the significance threshold multiple times across snapshots — the clearest demonstration in this document that a small effect sitting close to the detection threshold will keep crossing it as n changes. Read the underlying pattern as a stable, small, directionally-consistent tendency that this corpus size cannot reliably confirm or reject — not as a relationship that comes and goes.

---

### Finding G: JD authorship predicts stated dbt requirement (χ², p<0.0001, V=0.25, n=377)

Full cross-tab in §4.6. Hiring-manager-authored JDs name dbt at 73% vs. 37% for recruiter-authored — still the clearest authorship-quality signal in the dataset, with the gap and effect size essentially stable against the n=360 reading (V=0.25). Directly relevant to the dbt-prevalence caveat in §4.8 (recruiter-authored non-mentions of dbt are lower-fidelity evidence than hiring-manager non-mentions).

**Geography's link to JD authorship remains below p<0.05 (χ², p=0.016, V=0.15, n=389):** APAC roles are 88% hiring-manager-authored vs. 68% for the rest of the corpus, essentially unchanged from the n=372 reading (p=0.017, §9.5). The direction has been consistent throughout and this remains one of the clearer geographic effects in the dataset, though its p-value has crossed the conventional line in both directions across earlier snapshots — a small effect sitting close to the detection boundary, not a settled one. Two readings remain plausible and the JD text alone can't distinguish them: APAC hiring managers may write JDs more directly (less recruiter/ATS-template mediation in this sample), or the `jd_authorship` codebook's technical-specificity heuristic may be picking up an ATS-formatting convention specific to how these postings were sourced (many via LinkedIn/company career pages with detailed bullet-point tool lists) rather than true authorship. Given `jd_authorship`'s already-low self-consistency (0.58, §3), this finding should be read as a real, currently-significant pattern that has moved across the threshold before and could again, not as a settled claim about who actually writes APAC job postings.

---

### Finding H: Work arrangement — driven almost entirely by geography, with a maturity effect and an APAC disclosure signature that both remain significant (n=427 total / 389 analytical cohort)

A chi-square sweep of `work_arrangement` (hybrid / remote / onsite; `not_stated` excluded, 38% of the analytical cohort) against all other categorical and boolean dimensions found essentially one dominant driver: **where the job is**. `geo_region` remains by far the strongest association (χ²=120.37, p<0.0001, V=0.51, n=240 stated — direction and magnitude consistent with earlier snapshots) — remote roles concentrate almost entirely in `global_remote` and `uk_remote`, hybrid dominates every other region (Berlin, Iberia, Benelux, Nordics, France). This is close to tautological (a posting tagged "global remote" is remote by construction of the label) and the test remains statistically unreliable at the sparse-cell level given 14 regions × 3-4 arrangement categories. Treat the direction as real, the p-value as decorative.

**APAC's own signature remains significant on both cuts of the question.** Of the 50 APAC roles, 48% state no work arrangement at all, vs. 37% for the rest of the corpus — directionally the same read as every prior snapshot. APAC's *stated* arrangements continue to show a meaningfully higher onsite share (23% vs. 6% for the rest of the corpus) alongside a lower hybrid share (73% vs. 76%, closer to parity than earlier snapshots but still onsite-skewed overall). Crossing the full four-category `work_arrangement` breakdown (hybrid/not_stated/onsite/remote) against APAC-vs-rest gives χ²=11.99, p=0.007, V=0.18, n=389 — a small effect, on the significant side of the line, essentially unchanged from the n=372 reading (χ²=11.81, p=0.008, V=0.18). Read this as APAC's work-arrangement profile (both what gets disclosed and, when disclosed, what it says) continuing to measurably differ from the rest of the corpus, not just directionally suggestive of it. Among the 26 APAC roles that do state an arrangement, hybrid still dominates numerically (19 of 26) but the onsite share (6 of 26) remains the largest of any region in the dataset.

**The `data_team_maturity` relationship, on the stated-arrangement subset, remains significant: χ²=12.89, p=0.012, V=0.16 (n=240 stated)** — essentially unchanged from the n=372 snapshot's χ²=12.48, p=0.014, V=0.16, and still clears the threshold. Mature teams post hybrid most often (88% of stated arrangements) vs. 59% for early-stage teams, who split more evenly across hybrid/remote/onsite (59% / 23% / 18%); mid-stage sits between the two (75% / 19% / 6%). The direction is identical to every prior snapshot and matches the §4.3 maturity story — mature teams have converged on an operating default, early-stage teams are still deciding theirs. Interactive cross-tab and full write-up live in the dashboard (`index.html`, "Team maturity × Work arrangement" panel).

**`autonomy_level` × `work_arrangement` remains marginally significant: χ²=9.92, p=0.042, V=0.14 (n=240 stated)** — essentially unchanged from the n=372 reading (p=0.039, V=0.15), and the pattern has shifted somewhat: mixed-autonomy roles are now the most hybrid-concentrated (86%), while strategic roles show the highest remote share (23%) and execution roles the highest onsite share (9%). The effect size still sits right at the small-effect floor (V=0.14) — worth treating as a small, currently-real relationship rather than a headline finding, and one to keep re-checking rather than build a large claim on.

**On the missing 38% itself:** rather than just excluding `not_stated`, it's worth showing it as its own category, because it's an interesting result in its own right. Across maturity tiers it still does not concentrate strongly — mature (40%), mid (38%), and early-stage (33%) withhold a policy at close to the same rate, essentially unchanged from earlier snapshots. Folding `not_stated` back in as a fourth category for the maturity test (rather than excluding it) remains significant, though more weakly, at roughly the same level as the n=372 reading (χ²≈13.5, p≈0.036, V≈0.13, n=389). This is a different question than the stated-only test above ("does maturity predict whether an arrangement is stated at all," answer: modestly, yes, though weakening slightly) and both readings remain legitimate. "Does geography predict whether an arrangement is stated at all" remains confirmed at conventional thresholds for APAC (above) — and the dashboard panel shows the maturity views.
...
### Finding I: With `ai_role`, `testing_framing`, and `loss_aversion_framing` coded on the full cohort (n=389; §9.3), a systematic sweep against every other categorical dimension and tool flag surfaces several relationships, all stable at the current n

**Testing accountability tracks the fear register closely (χ²=91.86, p<0.0001, V=0.34, n=389):**

| testing_framing | high | moderate | none |
|---|---|---|---|
| absent (n=92) | 10% | 35% | 55% |
| responsibility (n=240) | 28% | 63% | 9% |
| tool_listed (n=57) | 7% | 65% | 28% |
...
|---|---|---|---|---|
| velocity_vs_rigour × domain_risk (Prediction 1) | χ² | 0.0002 | 0.17 | Stable at n=389 (was p=0.0001, V=0.18 at n=372) — small real effect, high-risk roles more rigour-dominant |
| velocity_vs_rigour × has_dbt (Prediction 1 comparator) | χ² | 0.143 | 0.10 | **Still not significant at n=389** — point estimate holds close to a 9pp rigour-rate gap, effect size below the small-effect floor — see §4.0 |
| ai_role × stakeholder_orientation (Prediction 2) | χ² | 0.21 | 0.12 | Still not significant at n=389 (was p=0.15 at n=372) — stable non-result |
| domain_risk × stakeholder_orientation | χ² | <0.0001 | 0.41 | Strongest relationship: finance concentrates high-risk, low-risk concentrates internal_data |
| data_team_maturity × greenfield_vs_fix | χ² | <0.0001 | 0.50 | Near-deterministic and stable: early=greenfield, mature=fix/scale |
| domain_risk × greenfield_vs_fix | χ² | 0.225 | 0.09 | **Still not significant at n=389** (was p=0.220, V=0.09 at n=372) — this relationship has crossed the threshold in both directions across earlier snapshots and remains null; read as a marginal effect this corpus size can't reliably detect either way, not a reversal (§4.9 Finding B) |
| jd_authorship × has_dbt | χ² | <0.0001 | 0.25 | Hiring-manager JDs name dbt ~2.0× more than recruiter JDs — stable |
| geo_region (APAC vs. rest) × jd_authorship | χ² | 0.016 | 0.15 | **Remains below p<0.05 at n=389** (was p=0.017, V=0.15 at n=372) — direction unchanged (88% vs. 68% hiring-manager-authored); Finding G, §9.5 |
| geo_region (APAC vs. rest) × work_arrangement (4-category) | χ² | 0.007 | 0.18 | **Remains significant at n=389** (was p=0.008, V=0.18 at n=372) — APAC's not-stated rate (48% vs. 37%) and its stated-arrangement mix (more onsite, less hybrid) both continue to measurably differ from the rest of the corpus; Finding H, §9.5 |
| seniority × autonomy_level | χ² | <0.0001 | 0.31 | Significant overall; "Senior" (n=153, still not the modal title by count — "Mid" is, at n=197) still predicts strongly (48% strategic) |
| stakeholder_orientation × autonomy_level | χ² | 0.168 | 0.12 | **Still not significant at n=389** (was p=0.152, V=0.13 at n=372) — this relationship has crossed the threshold multiple times across snapshots; read as a stable, small, directionally-consistent effect this corpus size can't reliably confirm (§4.9 Finding E) |
...
| collaboration_width × data_team_maturity | — | — | — | Still does not support a claim at n=389 (§4.7) |
...
| work_arrangement × data_team_maturity (stated subset) | χ² | 0.012 | 0.16 | **Still significant at n=389** (was p=0.014, V=0.16 at n=372) — same direction as every prior snapshot (mature teams skew hybrid) (Finding H) |
| work_arrangement × autonomy_level (stated subset) | χ² | 0.042 | 0.14 | **Still marginally significant** (was p=0.039, V=0.15 at n=372) — mixed-autonomy roles are now the most hybrid-concentrated, strategic roles show the highest remote share, effect sits right at the small-effect floor (Finding H) |
| work_arrangement × everything else (tool stack, seniority, rigour, domain risk) | χ² | >0.20 | ≤0.13 | Null — unrelated to arrangement |
| loss_aversion_framing × domain_risk | χ² | <0.0001 | 0.41 | 72% of high-loss-aversion JDs are high-domain-risk (equivalently: 60% of high-domain-risk roles carry high loss-aversion framing) (Finding I) |
...
| loss_aversion_framing × velocity_vs_rigour | χ² | <0.0001 | 0.36 | Cleaner than domain_risk's own link to rigour — 98% of high-loss-aversion JDs are rigour-framed (Finding I) |
| testing_framing × has_dbt | χ² | <0.0001 | 0.34 | dbt JDs 77% likely to frame testing as owned responsibility vs. 38% without dbt (Finding I) |
...
| ai_role × autonomy_level | χ² | <0.0001 | 0.24 | Unexpected direction, stable: `ai_user` roles are the most strategic-leaning of the three groups, not the least (Finding I) |
...
`ai_role` classifies whether the JD expects the candidate to *use* AI tools, *build* infrastructure AI systems consume, or neither. **Coded on the full analytical cohort (n=389)** — a bug in `scripts/write_jd.py` had silently dropped this field (and `testing_framing`, `loss_aversion_framing`) from JSON output for a long stretch of the corpus even when correctly classified; the backlog was fully re-coded against the JD archive text and the codebook (§9.3).
...
This is Prediction 2 from §4.0. **66% of JDs expect no AI skill from the candidate**, essentially flat against the n=372 reading (67%), against the dbt 2026 report's claim of 72% *daily* AI coding use among survey respondents. The gap between claimed personal-workflow adoption and formal hiring criteria has held steady. χ² for `ai_role` × `stakeholder_orientation` (n=389) remains non-significant (p=0.21, V=0.12; §4.0) — the `ai_enabler` cohort still leans toward `internal_data` (51%) and `mixed` (20%) stakeholder orientation, and `ai_user` leans more toward `mixed`/`commercial` (28%/11%), and the association's weakness is stable rather than trending toward zero.

**Actionable read:** `ai_enabler` roles → demonstrate data infrastructure built specifically for AI consumption. `ai_user` roles → demonstrate fluency with AI coding tools directly (Copilot, Claude Code, Cursor) as a nontrivial minority expectation. `none` (still the majority at 66%) → AI tool fluency is not a stated differentiator; leading with it misreads what's being screened for.

`ai_role` continues to track `autonomy_level` (χ²=45.52, p<0.0001, V=0.24) and `greenfield_vs_fix` (χ²=34.49, p<0.0001, V=0.21) at n=389 — see Finding I (§4.9) for the counter-intuitive direction (`ai_user` roles skew *more* strategic and *more* greenfield, not less). Both relationships are stable in direction against the n=372 reading, with both effect sizes essentially unchanged.

---

### 4.11 Testing framing: governance accountability is a majority hiring criterion

`testing_framing` distinguishes whether testing/data quality appears as something the candidate *owns*, a listed tool, or absent. **Coded on the full analytical cohort (n=389)** — see §9.3 for the write-pipeline bug that delayed this.

| testing_framing | n | % (n=389) |
|-----------------|---|---|
| responsibility | 240 | 62% |
| absent | 92 | 24% |
| tool_listed | 57 | 15% |

**62% of JDs frame testing as an owned responsibility** — action verbs (own, ensure, define, implement) paired with quality/data-contracts/observability language, unchanged from 62% at n=372. This is the clearest confirmation in the dataset of dbt 2026's "trust gap" narrative at the level of formal hiring criteria, distinct from §4.1's rigour finding: two rigour-coded JDs can differ in whether the *individual hire* is personally accountable for quality or whether it's team culture. `testing_framing = responsibility` identifies the former. `testing_framing × velocity_vs_rigour` is significant (χ²=43.27, p<0.0001, V=0.24, n=389): `responsibility`-coded JDs are 82% rigour-framed vs. 51% for `absent`-coded JDs — testing ownership and rigour framing move together but are not the same signal, since a substantial share of the cohort is rigour-framed with no testing-ownership language at all.
...
`testing_framing`'s strongest tool-stack link (`has_dbt`, χ²=45.67, p<0.0001, V=0.34) and its link to `jd_authorship` (χ²=48.48, p<0.0001, V=0.25) and `loss_aversion_framing` (χ²=91.86, p<0.0001, V=0.34) all hold stable at n=389 — see Finding I (§4.9) for detail. APAC roles run somewhat above the corpus average on `responsibility` framing (65% vs. 62% for the rest of the corpus), consistent with the n=372 snapshot; §9.5.

---

### 4.12 Loss-aversion framing: the market fears operational failure, not AI hallucinations

`loss_aversion_framing` classifies what the JD is afraid of: nothing, operational failure (outages, SLOs), or compliance/stakeholder-trust failure. **Coded on the full analytical cohort (n=389)** — see §9.3.

| loss_aversion_framing | n | % (n=389) |
|-----------------------|---|---|
| moderate | 221 | 57% |
| none | 88 | 23% |
| high | 80 | 21% |

Roughly three in four JDs carry some fear signal, but it's still predominantly operational (57%), not the compliance/AI-trust framing the dbt 2026 report leads with (71% citing fear of hallucinated outputs). `high` loss-aversion framing sits at 21%, essentially unchanged from n=372 (20%) — this batch's five high-domain-risk fintech/crypto roles (Ebury, Coins.ph, Super Payments, BCB Group, NOBA Bank Group) nudged it up fractionally, consistent with the relationship below. `loss_aversion_framing × domain_risk` is the strongest relationship among these three dimensions (χ²=131.4, p<0.0001, V=0.41, n=389, essentially unchanged from n=372): of JDs with `high` loss-aversion framing, 72% are `high`-domain-risk, vs. 15% of `moderate`-risk and 0% of `low`-risk roles carrying `high` framing — the fear register tracks real domain stakes closely, which is reassuring for the codebook's construct validity on this dimension. `high` loss-aversion framing remains concentrated in finance-adjacent and regulated-sector roles; APAC's own `high` rate (14%) remains below the corpus average this snapshot, consistent with the n=372 reading (§9.5).
...
Everything above classifies each JD as a whole against the ten Layer B dimensions. A separate pipeline (`analysis/responsibility_taxonomy.py`) takes a different cut of the same corpus: it parses just the responsibilities section of each JD (markdown headings, plain-text scrapes, condensed paragraph summaries, or — for JDs with no cleanly-parseable structure — an LLM-interpreted fallback, see `responsibility_bullets_llm.json`) into individual bullets, then keyword-classifies each bullet against a fixed 16-theme taxonomy (Data Modeling & Transformation, Stakeholder Collaboration, Mentorship & Leadership, AI & Agentic Workflows, and so on — full definitions and the keyword pattern behind each theme are in `analysis/responsibility_taxonomy.md`). 424 of the 427 total JDs (387 of the 389 analytical-cohort JDs) have a theme reading, extracting 2,942 bullets. Because each theme is a binary per-JD indicator, it can be crossed against any Layer B dimension as an ordinary 2×k contingency table — the question this section asks is which *specific responsibilities* go with which *behavioural traits*, not just which traits co-occur with each other (§4.9).

The most prevalent themes are Stakeholder Collaboration & Requirements (78% of parsed JDs), Data Modeling & Transformation (78%), Data Quality & Testing (68%), BI & Reporting/Dashboards (68%), and Governance & Documentation (60%). AI & Agentic Workflows sits at 30% — the sixteenth theme, added after an earlier corpus audit found the original 15-theme taxonomy had no bucket for AI-referencing responsibility bullets despite over a third of JDs containing them (see `project_responsibility_taxonomy_ai_gap` in the maintenance history); it now tracks closely with the Layer B `ai_role` dimension by construction (§ construct-overlap below).

**The auto-correlation risk, and how it's handled.** Several theme/dimension pairs are excluded from the findings below because they're circular, not because they're weak — the theme's regex keywords and the dimension's own LLM coding rubric detect the same textual signal. The single strongest pairing in the entire sweep, "AI & Agentic Workflows" vs. `ai_role` (V=0.66), is excluded on exactly this basis, followed closely by "Data Quality & Testing" vs. `testing_framing` (V=0.51): `testing_framing` is coded by looking for testing/quality language in the JD, so crossing it against a theme built from testing/quality keywords mostly measures whether two classification methods agree with each other. The same logic excludes "Security, Privacy & Risk" vs. `domain_risk`/`loss_aversion_framing`, "Data Ownership" vs. `autonomy_level` (whose own rubric lists "own" as a strategic-verb signal), and any theme against a tech-stack flag whose tool name is literally embedded in that theme's regex (dbt, Looker, Tableau, Power BI, Snowflake, Databricks, Airflow). See `OVERLAP_PAIRS` in `responsibility_taxonomy.py` for the full list.
...
1. **Mentorship & Leadership × `autonomy_level` (χ²=18.62, p<0.0001, V=0.22, n=387) — the one relationship in this section that received an actual confounder check, and passed it.** Mentorship/leadership language climbs from 6% of execution-coded JDs to 14% mixed to 26% strategic. Because `autonomy_level` and seniority title are themselves correlated (§4.5), this could just be seniority in disguise — so it was re-tested within seniority strata specifically: **within "Mid" titles alone**, the gradient is 3%→12%→9% (execution→mixed→strategic, noisy at these cell sizes); **within "Senior" titles alone**, it's 13%→14%→21%. The gradient is directionally consistent within the senior stratum specifically; the mid stratum is noisier at this n, so read the within-seniority claim as suggestive rather than as clean as the unstratified figure implies.
2. **Data Infrastructure & Warehouse Ops × `jd_authorship` (χ²=21.38, p<0.0001, V=0.24, n=387) — clean-screen only, not independently confounder-checked.** Hiring-manager-authored JDs name warehouse/infrastructure responsibilities at 57% vs. 13% for recruiter-authored — a considerably wider gap than authorship's already-known link to whether dbt is merely named (§4.6, §4.9 Finding G). Directionally consistent with the revealed-preference logic elsewhere in this document (naming a platform's actual cost/governance responsibilities requires knowing the team's real infrastructure problem, not just its tool list), but this specific pairing has not been re-tested against a plausible confounder the way (1) was.

**Self-Service Enablement & Data Literacy × `data_team_maturity` does not clear the p<0.01 screen and is not featured** — it no longer appears in the clean-relationships list at all at current corpus size, consistent with the earlier n=370 reading where it fell just short (χ²=4.46, p=0.11, V=0.11).

**A relationship that looked real and didn't survive scrutiny — kept as a worked example, not dropped:** `Architecture & Platform Strategy × work_arrangement` at one point cleared the p<0.01 screen and on its own read as a headline — "remote roles carry less architectural scope." A taxonomy audit corrected several loose keywords in this theme (bare "scalable"/"strategic" were dropped; "data platform" and "data lake" phrasing were added — see `responsibility_taxonomy.py`), and the pairing **does not clear the screen at current corpus size** (χ²=6.59, p=0.09, V=0.13, n=387; overall: hybrid 54%, remote 46%, onsite 26%, not_stated 45%) — it was never a stratification failure story to begin with; it was a keyword-precision artifact of the looser pre-audit pattern. The stratification breakdown is kept below as a worked example of *why* a stratification check matters, but the more direct lesson from this specific relationship turned out to be about pattern precision, not confounding:

- Within `data_team_maturity=early`: hybrid 39% (n=23); remote 22% (n=9); onsite 43% (n=7); not_stated 47% (n=19)
- Within `data_team_maturity=mid`: hybrid 51% (n=108); remote 54% (n=26); onsite 11% (n=9); not_stated 39% (n=89)
- Within `data_team_maturity=mature`: hybrid 66% (n=50); remote 50% (n=4); onsite 33% (n=3); not_stated 55% (n=40)
...
| dbt 2026 claim | JD evidence (n=389 analytical cohort) | Assessment |
|----------------|-------------|------------|
| 83% prioritise data trust | 71% rigour-oriented; testing framing coded on full cohort (n=389, 62% responsibility framing) | Confirmed at the orientation level and at the testing-accountability level, essentially unchanged from n=372 |
| 72% use AI in coding workflows daily | 66% of JDs (n=389, full coverage) expect no AI skill; 14% name AI coding tools directly (`ai_user`) | Gap persists, essentially unchanged from the n=372 reading (67%/13%) — Prediction 2 (§4.0), first half still holds, second half (structural concentration) remains non-significant |
| AI adoption outpacing governance (72% vs. 24%) | Governance accountability (62% of n=389 full cohort); AI hiring signal ~34% (`ai_enabler`+`ai_user`) | The JD evidence still suggests governance accountability further institutionalised than AI hiring criteria, and the ratio has held stable |
| Fear of hallucinated outputs (71%) | `loss_aversion_framing = high` is 21% of n=389 full cohort; 57% report operational reliability concerns | Not confirmed — dominant fear is still operational reliability, not AI-trust hallucination; both figures essentially unchanged from the n=372 reading |
| Rigour framing tracks risk/stakes | χ²=21.53, p=0.0002, V=0.17 (§4.0/§4.2, Prediction 1) — stable at n=389 | Confirmed for `domain_risk`; the `has_dbt` comparator that previously ran alongside it (§4.0) continues to show no detectable effect, which if anything sharpens this reading — rigour tracks risk, not tool adoption; rigour still close to universal (59%+) in every risk tier |
| dbt is the field standard | 66% of AE/BI JDs mention dbt (n=377) | Real but not universal; roughly one in three AE/BI roles run dbt-free stacks; stable across ten consecutive snapshots, including in the APAC subset specifically (66% — close to the corpus average; §9.5) |
...
**Current state: all three dimensions are coded on the full analytical cohort (n=389, including all JDs added since the fix, surviving both 2026-07-25 dedup passes, and every subsequent expansion through 2026-08-06, §9.9–§9.10)**, with consistent `evidence.{dim}` (quote) + `evidence.{dim}_explanation` (reasoning) entries on every record, and no legacy-format duplication. Findings in §4.10–4.12 and Prediction 2 (§4.0) are stated against the full current n, not a small coded subset — this changed several conclusions materially when the fix first landed at n=272 (§4.0, §4.10), and the corpus has grown, been deduplicated twice, and grown again multiple times since without disturbing that fix.

### 9.4 What n=389 supports

At n=389, the margin of error on a single proportion is approximately ±4.5pp at 95% confidence (Wilson interval) — the 71% rigour finding (§4.1) is defensible as "likely between 66% and 75%," not as a precise market figure. Cross-tabs with cell sizes below ~15 (junior seniority, pure velocity, low domain-risk in some cross-tabs) are illustrative, not evidential, and are flagged as such at each occurrence above. The corpus additions from the n=123 baseline through n=389 provided meaningful confidence-interval tightening and, along the way, flipped several relationships across the significance threshold in both directions (§4.0, §4.9 document each one as it happened). The most recent +17-record expansion (§9.10) is modest and left every headline distribution within noise of its pre-batch value (rigour 71.0% unchanged, domain_risk moderate 68.4% unchanged, has_dbt 65.5% unchanged, ai_role=none 66.1%, down ~1pp) — see §9.10 for the batch's own composition. The three relationships that tested null at n=340 — `velocity_vs_rigour × has_dbt`, `domain_risk × greenfield_vs_fix`, `stakeholder_orientation × autonomy_level` — all remain null at n=389. Pattern stability continues to hold for the strongest relationships (maturity × mission, domain_risk × stakeholder, domain_risk × rigour) across every snapshot from n=123 to n=389.
