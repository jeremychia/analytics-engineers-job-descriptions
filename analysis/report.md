...
**Test:** `ai_role` is coded across the full analytical cohort (n=527 — see §9.3 for the pipeline-bug history; a further evidence-quality audit on 2026-08-23 found and corrected several `ai_role` misclassifications, see §9.16). `ai_role = none` is 62% of the cohort (down from 65% at n=502) — against the dbt 2026 report's claim of 72% *daily* AI coding use. χ² for `ai_role` × `stakeholder_orientation` (n=527): χ²=12.34, p=0.137, V=0.11 — **not significant**, and further from the conventional threshold than the prior reading (was p=0.058). `ai_enabler` (118 of 527, 22%) still concentrates somewhat in `internal_data` and `mixed` stakeholder orientation versus `ai_user` (82, 16%) which spreads similarly. **Prediction 2's second half (non-random concentration) continues to not hold at conventional significance; the first half (large adoption-claim/hiring-criterion gap) still holds — `none` is 62% of hiring criteria against the survey's 72% daily-use claim.**
...
This is Prediction 2 from §4.0. **62% of JDs expect no AI skill from the candidate**, down from 65% at n=502, against the dbt 2026 report's claim of 72% *daily* AI coding use among survey respondents. The gap between claimed personal-workflow adoption and formal hiring criteria has held steady. χ² for `ai_role` × `stakeholder_orientation` (n=527) remains non-significant (p=0.137, V=0.11; §4.0) — the `ai_enabler` cohort still leans toward `internal_data` and `mixed` stakeholder orientation, and `ai_user` leans similarly, and the association's weakness is stable rather than trending toward zero. A 2026-08-23 evidence-quality audit found and corrected several `ai_role` misclassifications (§9.16) — 7 records were relabeled (mostly `none`→`ai_enabler`/`ai_user` where a JD's own responsibilities named a specific AI-tool or AI-infrastructure expectation that an earlier coding pass had missed); the shift moved every number in this table by 1pp or less.
...
`ai_role` continues to track `autonomy_level` (χ²=49.89, p<0.0001, V=0.22) and `greenfield_vs_fix` (χ²=32.41, p<0.0001, V=0.18) at n=527 — see Finding I (§4.9) for the counter-intuitive direction (`ai_user` and `ai_enabler` roles skew *more* strategic and *more* greenfield, not less).
