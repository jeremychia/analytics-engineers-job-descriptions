...
**Honest scope limitations:** 478 JDs (analytical cohort) is a moderate-scale dataset with tighter confidence intervals than earlier snapshots. The confidence interval on a single proportion is approximately ±4.1pp at 95% (Wilson interval) — tight enough that core dimensions (rigour, domain_risk, maturity) show directional consistency, but still wide enough that individual percentages should be read as directional signals, not precise market measurements. The geographic concentration is still primarily European/Berlin, but the APAC stratum (n=69) remains large enough to test directly against the European majority rather than merely disclaim — see §9.5 for what that comparison shows and its own, tighter limits. Generalisation to North America remains effectively untested — the 2026-08-13 batch introduced a small `nyc_metro` cluster (n=29, §3, §9.12) but it is far too small and too recently added to support any US-specific comparison; treat it as corpus-coverage information, not a North American stratum on par with APAC or Europe. These limitations are stated once here and apply to every finding in this document; they are not repeated at every mention. Mid-corpus expansions (July 13, 2026: +9 JDs; July 16, 2026: +12 JDs; July 17, 2026: +13 JDs; July 21, 2026: +21 JDs; July 22–24, 2026: +55 JDs; July 26–29, 2026: +38 JDs; July 30, 2026: +11 JDs; August 5, 2026: +15 JDs; August 6, 2026: +17 JDs; August 11, 2026: +17 JDs; August 13, 2026: +18 JDs to the analytical cohort, §9.9–§9.12) added new roles without statistical re-weighting, so updated findings through that point reflected raw inclusion in the analytical cohort. The 2026-07-25 dedup removed 37 duplicate records across two passes rather than adding new ones (§3, §9.6) — this tightened the corpus rather than diluting it. Every relationship in this document is re-tested at each corpus update, and significance is not treated as permanent: this revision (n=478) finds `domain_risk × greenfield_vs_fix` (previously Finding B, χ²=6.21, p=0.18, V=0.08) still does not clear p<0.05 — restated below as a current null. `stakeholder_orientation × autonomy_level` (previously Finding E), null at n=440 (p=0.175), now sits just outside p<0.05 on the significant side (χ²=14.96, p=0.060, V=0.13) — not yet confirmed, flagged as one to watch. `velocity_vs_rigour × has_dbt` (§4.0's Prediction 1 comparator), which first crossed p<0.05 at n=427, remains significant with a wider margin this revision (χ²=8.39, p=0.015, V=0.13, n=462 AE/BI). `geo_region (APAC) × jd_authorship` (previously part of Finding G), null at n=440 (p=0.071), crosses back to significant this revision (χ²=6.04, p=0.014, V=0.11) — restated plainly as a current finding, not a "recovery." `data_team_maturity × work_arrangement` on the stated-arrangement subset remains significant at this n (χ²=19.78, p=0.0006, V=0.18, n=302 stated).
...
**531 job descriptions** collected April–August 2026 across `data/`, deduplicated 2026-07-25 in two passes (§9.6). Role-type breakdown:

| role_type | n | In scope |
|---|---|---|
| analytics_engineering_bi | 462 | Yes — primary cohort |
| team_lead | 16 | Yes — governance-signalling stratum |
| data_engineering | 44 | No — excluded, different discourse population |
| other | 9 | No — excluded |

**Analytical cohort: 478 records** (AE/BI + team_lead). Team-lead roles are retained because they are the most likely to contain explicit governance-mandate language ("define testing standards", "establish data culture") — relevant to whether the 2026 report's governance anxiety has entered hiring language at the decision-making level, not just the individual-contributor level.

**Geographic spread:** Primarily European (Benelux 11%, UK/remote 13%, Iberia 9%, Berlin 7%, France 7%, Nordics 7%, other Europe 7%), with APAC remaining the largest single bucket (**69 roles, 14.4% of the analytical cohort**) — large enough to compare directly against the European majority rather than merely disclaim as a blind spot (§9.5). The 2026-08-13 batch (§9.12) introduced a new `nyc_metro` bucket (29 roles, 6.1%) reflecting a cluster of New York-based postings, the corpus's first meaningful US-specific geographic concentration. The `geo_region` field is a keyword match against free-text `job_location` strings collected opportunistically during a job search — it describes what got scraped, not real market concentration. Treat regional splits as corpus-coverage information, not a labour-market claim. See §9.5 for a worked APAC-vs-Europe comparison, and §9.13 for a fuller three-way Europe/APAC/NYC-metro comparison run once the NYC cluster reached n=29 — it surfaces one large effect (`language_gate_type`) that §9.5's two-way framing never tested and that the seeker/manager-mode UI (`index.html`) does not yet surface at all.
...
**Test:** χ² for `velocity_vs_rigour` × `domain_risk` (n=462, AE/BI only): χ²=30.20, p<0.0001, V=0.18 — stable, essentially identical to the n=440 reading (p<0.0001, V=0.19). χ² for `velocity_vs_rigour` × `has_dbt` (n=462, AE/BI only): χ²=8.39, **p=0.015, V=0.13 — remains significant, and more comfortably so than the n=427 reading (p=0.047, V=0.12).** This comparator sat just below the conventional threshold for many consecutive snapshots before first crossing it at n=427 (p=0.14 at n=389, p=0.058–0.143 across the batches before that); this revision holds the crossing with more room to spare rather than drifting back toward the line, suggesting the earlier crossing was not purely a one-off artifact of a borderline effect. **This continues to complicate Prediction 1's clean reading:** rigour framing shows a small, detectable association with both `domain_risk` (V=0.18) and `has_dbt` (V=0.13) — the two remain not cleanly separated by significance, though `domain_risk`'s effect remains the larger of the two. Prediction 1's directional claim (risk matters at least as much as tool adoption) still holds; its stronger claim (tool adoption shows *no* detectable link) continues to not hold at this n. High-risk roles remain markedly more rigour-dominant (§4.2), a real gradient, stable across every snapshot from n≤240 onward.
...
**Test:** `ai_role` is coded across the full analytical cohort (n=478 — see §9.3 for the pipeline-bug history). `ai_role = none` is 65% of the cohort (unchanged from n=440) — against the dbt 2026 report's claim of 72% *daily* AI coding use. χ² for `ai_role` × `stakeholder_orientation` (n=478): χ²=12.44, p=0.13, V=0.11 — **remains not significant**, consistent with every prior reading. `ai_enabler` (98 of 478, 21%) still concentrates somewhat in `internal_data` and `mixed` stakeholder orientation versus `ai_user` (71, 15%) which spreads similarly, and the association's weakness is stable rather than still trending toward zero. **Prediction 2's second half (non-random concentration) continues to not hold at conventional significance; the first half (large adoption-claim/hiring-criterion gap) still holds — `none` is 65% of hiring criteria against the survey's 72% daily-use claim.**

**What this buys the document:** two explicit, checkable predictions, stated before the findings that test them, with the statistical result reported honestly as it changes — including when a corpus expansion moves an earlier reading, as happened here. Prediction 1's `domain_risk` comparator has held significant at essentially the same effect size from n=272 through the current n=478; its `has_dbt` comparator, non-significant across every snapshot through n=389, crossed p<0.05 for the first time at n=427 and continues to hold at n=462 with a slightly wider margin (p=0.015, V=0.13) — a borderline effect that has now held across two consecutive corpus updates rather than a one-off crossing (see the Test paragraph above). Prediction 2 flipped a different direction earlier on: a marginal, medium-effect result at a small, biased coded subset (n=86) gave way to a clear non-result once the same three dimensions were coded across the full cohort, and that non-result has held at every subsequent n including n=478. These trajectories are instructive about statistical power and sample composition rather than embarrassing reversals to paper over — this is the fix for Appendix B's "six theories, none tested" critique — not a stronger claim than the data supports, but an honest, and honestly-updated, one.
...
| velocity_vs_rigour | n | % (analytical, n=478) |
|--------------------|---|---|
| rigour | 325 | 68% |
| mixed | 136 | 28% |
| velocity | 17 | 4% |

**68% of JDs in the analytical cohort signal a rigour orientation**, unchanged from n=440. Pure velocity holds at 4% (17 JDs across 478, unchanged in count from n=440 — the new batch added no velocity-coded roles). This remains the clearest single-dimension finding in the dataset by margin, and the 2026-08-13 batch (§9.12) — skewed toward larger, more established organisations (Fox Corporation, Siemens, Warner Bros. Discovery, Bristol Myers Squibb) — held the rigour share flat rather than continuing the prior drift downward. The overall trajectory (80% → 75% → 75% → 72% → 73% → 71% → 71% → 71% → 71% → 68% → 68%) shows the recent downward drift levelling off at this n, not reversing. Per §4.2, rigour framing shows a small but statistically real gradient with domain risk; per §4.0, its gradient with tool adoption (`has_dbt`) also clears significance at this n — see §4.0 for the full account of that shift.

This is broadly consistent with the dbt 2026 report's governance framing — but the consistency is directional, not mechanistic. The JD data cannot distinguish "rigour because of genuine engineering craft" from "rigour because of fashion diffusion" from "rigour because of fear of AI-generated errors." §4.0's test finds a small, real effect for risk, and now also a small, real effect for tooling — the two are no longer cleanly separated by significance at this n.
...
| domain_risk | n | % (analytical, n=478) |
|-------------|---|---|
| moderate | 323 | 68% |
| high | 124 | 26% |
| low | 31 | 6% |

**Cross-tab with velocity_vs_rigour** (AE/BI only, n=462):

χ²=30.20, p<0.0001, V=0.18 (n=462 — stable, essentially identical to the n=440 reading of p<0.0001, V=0.19). **High-risk roles remain detectably more rigour-dominant than moderate or low-risk roles,** and the relationship has held through every expansion and both dedup passes essentially unchanged in effect size. The new batch (§9.12) added seven high-risk roles (Aperia Solutions, Current, Bristol Myers Squibb, payabl., Zego, foodpanda, CMC Markets — several fintech/payments/insurance) and only one low-risk role (Secretlab), which is why the low-risk share edged down (7%→6%) rather than continuing its prior noise-driven increase. This still broadly confirms §4.0 Prediction 1's interpretation: domain risk carries the larger of the two effect sizes now that the `has_dbt` comparator has also crossed significance (§4.0). The effect size (V=0.18) stays in "small" territory — domain risk explains some but far from most of the variance in rigour framing. Read this as: rigour language is common everywhere but shifts upward, modestly and reliably, when the stakes of an error are genuinely higher.
...
| data_team_maturity | n | % (analytical, n=478) |
|--------------------|---|---|
| mid | 276 | 58% |
| mature | 128 | 27% |
| early | 74 | 15% |

**Just under three-fifths of roles are mid-stage.** Early-stage roles hold at 15%; genuinely mature organisations are 27% — both essentially unchanged from the n=440 snapshot (16%/26%). The new batch (§9.12) skewed toward mid- and mature-stage organisations (9 mid, 8 mature, 1 early — several large established employers: Fox Corporation, Warner Bros. Discovery, Siemens, N-able, CMC Markets), nudging the mature share up slightly. APAC's own maturity mix (§9.5) continues to track the corpus average closely.

**Maturity × greenfield_vs_fix cross-tab** (χ²=253.81, p<0.0001, V=0.52, n=478 — the strongest relationship in the dataset, and stable across every expansion and dedup pass):
...
Greenfield work concentrates sharply at early-stage (77%) and is nearly absent at mature (4%). This is the structural basis for the common career-advice claim "go early-stage for greenfield work," and it continues to hold cleanly — the strongest and most reliable relationship in the entire dataset, with the effect size essentially unchanged at V=0.52.
...
χ²=40.88, p<0.0001, V=0.21 (n=478, essentially unchanged from n=440's V=0.21). Early-stage roles offer strategic autonomy at 61% — still far above mid- or mature-stage roles (23% and 36%). Mid-stage remains the least strategic tier despite being the largest market segment; mature-stage's strategic share ticked up (30%→36%) with this batch's addition of several staff/lead-level mature-org roles (MUBI, Warner Bros. Discovery, N-able) that carry strategic framing despite being mature organisations. The core pattern — greenfield work and direction-setting cluster at early-stage companies — holds.
...
| stakeholder_orientation | n | % (analytical, n=478) |
|-------------------------|---|---|
| internal_data | 231 | 48% |
| mixed | 98 | 21% |
| commercial | 63 | 13% |
| finance | 51 | 11% |
| product | 35 | 7% |

**48% of roles in this cohort primarily serve internal data consumers** — other analysts, data scientists, ML engineers, or the platform itself. This remains the dominant archetype in the market, essentially unchanged from the n=440 reading (49%). APAC's own stakeholder mix isn't a standout finding — see §9.5.

**Cross-tab with rigour** (χ²=53.86, p<0.0001, V=0.24, n=478):

| stakeholder_orientation | mixed | rigour | velocity | n |
|-------------------------|-------|--------|----------|---|
| finance | 12% | 88% | 0% | 51 |
| internal_data | 19% | 78% | 3% | 231 |
| product | 40% | 54% | 6% | 35 |
| mixed | 45% | 52% | 3% | 98 |
| commercial | 44% | 46% | 10% | 63 |

Finance and internal_data roles remain the most rigour-dominant (78–88%); commercial and product roles are close to evenly split between rigour and mixed framing, with commercial's velocity share holding at 10% (unchanged from n=440). This relationship remains clearly significant at n=478 (V=0.24, essentially unchanged from n=440's V=0.26) — still the clearest stakeholder-level driver of rigour/velocity framing in the dataset. The new batch added five finance-oriented roles (Aperia Solutions, Current, Bristol Myers Squibb, foodpanda, CMC Markets — several fintech/payments), reinforcing rather than shifting the finance/rigour link.
...
| autonomy_level | n | % (analytical, n=478) |
|----------------|---|---|
| mixed | 184 | 38% |
| strategic | 154 | 32% |
| execution | 140 | 29% |

The three-way split persists, and `mixed` continues to lead `strategic`/`execution` (38% vs. 32%/29%, essentially unchanged from the n=440 reading of 39/31/30). This remains within the range this corpus size produces from batch to batch and is not read as a trend toward `mixed` overtaking the other two categories structurally — just where the current n happens to land. This even distribution reinforces that autonomy cannot be read from title or seniority label alone; context (maturity, stakeholder, domain risk) matters much more.

**Seniority × autonomy** (χ²=102.08, p<0.0001, V=0.33, n=478):
...
The relationship remains statistically real (p<0.0001, effect size essentially unchanged at V=0.33) and the practical read is stable: **"Mid" remains the single largest title cohort (n=242) but "Senior" (n=183) remains the more informative one, splitting 19/33/48 across execution/mixed/strategic — solidly more strategic-leaning than the corpus-wide split, consistent with prior snapshots.** A "Senior Analytics Engineer" title continues to be a meaningfully positive predictor of strategic scope, though it remains far from deterministic (a fifth of senior roles are still pure execution). Lead, manager, and staff titles predict strategic scope more clearly still (67–83%), but remain small cells — the new batch added three manager-level roles (Fox Corporation, Current, Bristol Myers Squibb) and three staff-level roles (Butterfly Network, Warner Bros. Discovery, N-able), all coded strategic or mixed, consistent with the existing pattern rather than shifting it. The practical implication for interviews is unchanged: ask explicitly what decisions the role makes autonomously in year one; the senior title is informative but still leaves real uncertainty.

---

### 4.6 JD authorship: hiring managers write roughly 70% of the corpus; the APAC gap is now a tested null
...
| jd_authorship | n | % (analytical, n=478) |
|---------------|---|---|
| hiring_manager | 355 | 74% |
| mixed | 90 | 19% |
| recruiter | 33 | 7% |

**Hiring-manager-authored JDs are 74% of the corpus**, essentially unchanged from 73% at n=440. **The APAC gap has crossed back above p<0.05 this revision:** APAC roles are 87% hiring_manager-authored vs. 72% for the rest of the corpus (χ²=6.04, p=0.014, V=0.11, n=478 — down from the n=440 reading of p=0.071, V=0.11). Per the "significance is not permanent" principle applied throughout this document (§1), this is stated plainly rather than as a "recovery" or "reversal": the point estimate barely moved (85%→87%) and the effect size is unchanged (V=0.11) — what changed is that the new batch's non-APAC roles skewed slightly more toward mixed/recruiter authorship (several large-organisation postings — Fox Corporation, Siemens, Warner Bros. Discovery — carry more templated HR-department language than the corpus average), which widened the gap on the non-APAC side rather than the APAC cell itself moving. This gap has now crossed the conventional threshold in both directions across several consecutive snapshots (§9.5's table shows the trajectory); treat the direction (APAC skews toward hiring-manager authorship) as the durable signal and the specific p-value as sensitive to which non-APAC roles happen to land in a given batch.

**Cross-tab with rigour** (χ²=9.60, p=0.048, V=0.10, n=478): hiring_manager 71% rigour / 25% mixed / 4% velocity; mixed 60% rigour / 38% mixed / 2% velocity; recruiter 55% rigour / 42% mixed / 3% velocity. This remains just under the conventional threshold on the significant side (was p=0.027 at n=440) — still a small effect (V=0.10) right at the detection floor, so read this as a marginal, borderline-detectable association between authorship sophistication and rigour framing, not a robust finding on the order of domain_risk's own link (§4.2).

**Cross-tab with has_dbt** (χ²=27.71, p<0.0001, V=0.24, n=462, AE/BI only):

| jd_authorship | has_dbt=False | has_dbt=True | n |
|---------------|---------------|---------------|---|
| hiring_manager | 29% | 71% | 341 |
| mixed | 51% | 49% | 88 |
| recruiter | 61% | 39% | 33 |

Hiring-manager-authored JDs name dbt at roughly 2× the rate of recruiter-authored ones (71% vs. 39%), essentially unchanged from the n=427 reading — the relationship is stable and the effect size held (V=0.24, down marginally from 0.25). Read against Deming & Kahn's revealed-preference framework (§6): a hiring-manager-named tool requirement is a higher-fidelity signal than a recruiter-named one — the manager screens for it because they use it; the recruiter may be pulling from a template. The practical implication: dbt's *absence* in a recruiter-authored JD is weaker evidence the team doesn't use it than absence in a hiring-manager-authored JD.
...
The earlier draft's finding — mature teams have the widest named-stakeholder count — is directionally intact (2.80 vs. 2.40 vs. 2.49), broadly consistent with the n=440 reading (2.73/2.38/2.51). Mature still leads and mid still trails, the same ordering as every prior snapshot, and early sits between the two as it has for several snapshots now rather than at either extreme. **This dimension still does not currently support a confident finding.** It is retained in the codebook for future corpus growth, but no claim built on it should be treated as established.

---

### 4.8 dbt prevalence: real but not universal

`has_dbt` is a required-or-preferred tool flag, not a Layer B dimension. **65% of AE/BI roles (n=462) mention dbt.**

This is consistent with dbt's own claim that it has become the field standard, but roughly one in three AE/BI roles run on a stack without it. The prevalence has held essentially flat across the last several snapshots (68%→66%→65%→65%→66%→65%→64%→65%→65%→66%→64%→65%), including through both dedup passes and every subsequent expansion — dbt prevalence among APAC AE/BI roles specifically remains close to the corpus average (§9.5). This market includes a meaningful share of Databricks SQL, BigQuery-native, and Spark-first stacks. A survey distributed exclusively through dbt's community channels cannot see that portion of the market by construction — this is the self-selection constraint from §2, made concrete. The JD data documents this blind spot directly: roughly one in three roles don't name dbt at all, stable across twelve consecutive corpus snapshots.
...
**Chi-squared (χ²):** applied to categorical × categorical pairs with adequate expected cell frequencies. At n=440, the minimum detectable effect (α=0.05, 80% power) for a typical cross-tab is Cramér's V ≈ 0.15 — marginally lower than the n=389 threshold (V≈0.16), consistent with the modest power gain from ~50 additional records. Findings below the current threshold are still directional only.
...
### Finding A: Domain risk and stakeholder orientation are structurally linked (χ², p<0.0001, V=0.38, n=478)

| domain_risk | commercial | finance | internal_data | mixed | product |
|-------------|-----------|---------|---------------|-------|---------|
| high (n=124) | 5% | 37% | 39% | 15% | 4% |
| low (n=31) | 3% | 0% | 74% | 19% | 3% |
| moderate (n=323) | 17% | 2% | 50% | 23% | 9% |

High-risk roles concentrate heavily in finance (~37%, vs. 0% of low-risk and ~2% of moderate-risk roles), unchanged in direction and essentially unchanged in effect size (V=0.38, unchanged from n=440) since the new batch's five finance-oriented high-risk roles (§4.4) reinforced rather than shifted this cell. Still the strongest, cleanest relationship in the dataset outside of maturity × mission (§4.3). Product-facing roles remain rare in high-risk contexts (4%) — experimentation and funnel work is essentially never coded high-stakes in this corpus, even though A/B test errors can carry real revenue consequences. Low-risk roles skew overwhelmingly `internal_data` (74%) — internal tooling and education-sector roles serve internal data consumers almost by definition.
...
### Finding B: domain risk and mission type — a tested null (χ², p=0.184, V=0.08, n=478)

| domain_risk | fix_scale | greenfield | mixed |
|-------------|-----------|-----------|-------|
| high (n=124) | 39% | 15% | 46% |
| low (n=31) | 45% | 16% | 39% |
| moderate (n=323) | 29% | 16% | 54% |

Moderate-risk roles still look the most "mixed" (incremental extension of an existing stack, 54%) and low-risk roles still lean somewhat more toward fix_scale (45%) than moderate-risk roles (29%) — the direction is broadly the same as prior snapshots, and the test remains well short of p<0.05 at this n, with the effect size ticking up slightly to V=0.08 (from 0.07 at n=440, within noise). This relationship has moved across the threshold in both directions across earlier snapshots and remains null at the current n — read that trajectory as a small, marginal effect that this corpus size can't reliably detect either way, not as evidence the underlying pattern has changed. Kept here as a documented test, not as a claimed finding.

---

### Finding C: Maturity determines mission almost deterministically (χ², p<0.0001, V=0.52, n=478)

Full cross-tab in §4.3. Greenfield work is 77% of early-stage roles and 4% of mature-team roles — the sharpest, most reliable relationship in the corpus, holding at V=0.52, essentially unchanged from n=440.
...
### Finding D: Seniority predicts autonomy moderately for the modal title, strongly at the tails (χ², p<0.0001, V=0.33, n=478)

Full cross-tab in §4.5. "Mid" (n=242) remains the largest title cohort by count, but "Senior" (n=183) remains the more informative title, spanning execution/mixed/strategic at 19/33/48 — noticeably more strategic-leaning than the corpus-wide split, consistent with prior snapshots. Staff, manager, and lead titles (n=12, n=9, n=15) predict strategic scope near-perfectly (67–83%), but the cells remain too small to generalise with confidence.

**Theoretical read — Spence (1973), signalling, now more mixed than contradicted:** if job titles were reliable, costly-to-fake signals, "Senior" should predict autonomy cleanly. At n=478 "Senior" remains a meaningfully informative signal (48% strategic vs. an overall cohort rate of 32%), consistent with prior snapshots — the signalling account continues to look less contradicted than the n=123 baseline suggested, though a fifth of senior roles remain pure execution, so the signal stays noisy. Staff/manager/lead titles retain the strongest signal value, consistent with being rarer and costlier to award, but the cells are too small here to treat as confirmed.

---

### Finding E: stakeholder orientation and autonomy level — now bordering significance (χ², p=0.060, V=0.13, n=478)

| stakeholder_orientation | execution | mixed | strategic |
|-------------------------|-----------|-------|-----------|
| finance (n=51) | 33% | 31% | 35% |
| commercial (n=63) | 19% | 48% | 33% |
| mixed (n=98) | 21% | 46% | 33% |
| internal_data (n=231) | 35% | 36% | 29% |
| product (n=35) | 29% | 26% | 46% |

Finance-facing roles still look among the more execution-concentrated segments (33%) and product-facing roles still look the most strategic (46%), the same direction as every prior snapshot, and the test has moved to just outside p<0.05 on the significant side this revision (p=0.060, up from p=0.175 at n=440), with the effect size ticking up (V=0.13, from 0.11). This relationship has crossed the significance threshold multiple times across snapshots — the clearest demonstration in this document that a small effect sitting close to the detection threshold will keep crossing it as n changes. Read the underlying pattern as a stable, small, directionally-consistent tendency that this corpus size cannot yet reliably confirm or reject — not as a relationship that comes and goes.

---

### Finding G: JD authorship predicts stated dbt requirement (χ², p<0.0001, V=0.24, n=462)

Full cross-tab in §4.6. Hiring-manager-authored JDs name dbt at 71% vs. 39% for recruiter-authored — still the clearest authorship-quality signal in the dataset, with the gap and effect size essentially stable against the n=440 reading (V=0.25→0.24). Directly relevant to the dbt-prevalence caveat in §4.8 (recruiter-authored non-mentions of dbt are lower-fidelity evidence than hiring-manager non-mentions).

**Geography's link to JD authorship has crossed back to significant (χ², p=0.014, V=0.11, n=478):** APAC roles are 87% hiring-manager-authored vs. 72% for the rest of the corpus — the same direction held across every prior snapshot (§9.5), and the gap has now moved back below the conventional threshold (was p=0.071 at n=440). Per this document's "significance is not permanent" convention, this is restated plainly as a current finding, not framed as a "recovery" of a result that was ever lost — the point estimate and effect size barely moved between the two snapshots (85%→87% APAC, V=0.11→0.11); what moved was the non-APAC comparison group, which this batch nudged slightly toward more mixed/recruiter-authored postings. Two readings remain plausible and the JD text alone can't distinguish them: APAC hiring managers may write JDs more directly (less recruiter/ATS-template mediation in this sample), or the `jd_authorship` codebook's technical-specificity heuristic may be picking up an ATS-formatting convention specific to how these postings were sourced (many via LinkedIn/company career pages with detailed bullet-point tool lists) rather than true authorship. Given `jd_authorship`'s already-low self-consistency (0.58, §3) and this relationship's history of repeatedly crossing the p<0.05 line in both directions across consecutive snapshots, treat the direction (APAC skews toward hiring-manager authorship) as the durable signal and the specific significance test as sensitive to corpus composition batch to batch.

---

### Finding H: Work arrangement — driven almost entirely by geography, with a maturity effect and an APAC disclosure signature that both remain significant (n=531 total / 478 analytical cohort)

A chi-square sweep of `work_arrangement` (hybrid / remote / onsite; `not_stated` excluded, 37% of the analytical cohort) against all other categorical and boolean dimensions found essentially one dominant driver: **where the job is**. `geo_region` remains by far the strongest association (χ²=149.71, p<0.0001, V=0.50, n=302 stated — direction and magnitude consistent with earlier snapshots) — remote roles concentrate almost entirely in `global_remote` (85% remote) and `uk_remote`, hybrid dominates every other region. The new batch introduced a `nyc_metro` bucket with a notably onsite-skewed profile (26% onsite, matching APAC's onsite share, vs. 5–8% for most European regions) — the batch's several New York-based roles (Aperia Solutions, Butterfly Network, Puig, Current, Fox Corporation) were mostly explicitly onsite or hybrid-with-office-days rather than remote. This is close to tautological (a posting tagged "global remote" is remote by construction of the label) and the test remains statistically unreliable at the sparse-cell level given 15 regions × 3 arrangement categories. Treat the direction as real, the p-value as decorative.

**APAC's own signature remains significant on both cuts of the question.** Of the 69 APAC roles, 49% state no work arrangement at all, vs. 35% for the rest of the corpus — directionally the same read as every prior snapshot. APAC's *stated* arrangements continue to show a meaningfully higher onsite share (26% vs. 8% for the rest of the corpus) alongside a lower hybrid share (69% vs. 77%). Crossing the full four-category `work_arrangement` breakdown (hybrid/not_stated/onsite/remote) against APAC-vs-rest gives χ²=15.51, p=0.0014, V=0.18, n=478 — a small effect, and stronger on this reading than the n=440 snapshot (χ²=10.99, p=0.012, V=0.16). Read this as APAC's work-arrangement profile (both what gets disclosed and, when disclosed, what it says) continuing to measurably differ from the rest of the corpus, and this revision's tighter significance suggests the pattern is stabilising rather than eroding. Among the 35 APAC roles that do state an arrangement, hybrid still dominates numerically but the onsite share (26%) remains among the largest of any region in the dataset, now matched by the new `nyc_metro` bucket.

**The `data_team_maturity` relationship, on the stated-arrangement subset, remains significant: χ²=19.78, p=0.0006, V=0.18 (n=302 stated)** — a slightly smaller effect size than the n=440 snapshot's V=0.20 (χ²=21.87, p=0.0002), within the range of batch-to-batch noise this test has shown before. Mature teams post hybrid most often (86% of stated arrangements) vs. 58% for early-stage teams, who split more evenly across hybrid/onsite/remote (58% / 23% / 19%); mid-stage sits between the two (77% / 7% / 16%). The direction is identical to every prior snapshot and matches the §4.3 maturity story — mature teams have converged on an operating default, early-stage teams are still deciding theirs. Interactive cross-tab and full write-up live in the dashboard (`index.html`, "Team maturity × Work arrangement" panel).

**`autonomy_level` × `work_arrangement` remains marginally significant: χ²=13.68, p=0.008, V=0.15 (n=302 stated)** — essentially unchanged from the n=440 reading (p=0.022, V=0.14), and the pattern is broadly stable: mixed-autonomy roles remain the most hybrid-concentrated (86%), while strategic roles show the highest onsite share (16%) and a matching remote share (18%) simultaneously — a polarised strategic-role profile consistent with the n=440 reading. The effect size still sits right at the small-effect floor (V=0.15) — worth treating as a small, currently-real relationship rather than a headline finding, and one to keep re-checking rather than build a large claim on.

**On the missing 37% itself:** rather than just excluding `not_stated`, it's worth showing it as its own category, because it's an interesting result in its own right. Across maturity tiers it still does not concentrate strongly — mature (43%), mid (36%), and early-stage (30%) withhold a policy at close to the same rate, essentially unchanged from earlier snapshots. Folding `not_stated` back in as a fourth category for the maturity test (rather than excluding it) remains significant, at a level consistent with the n=440 reading (χ²=24.47, p=0.0004, V=0.16, n=478). This is a different question than the stated-only test above ("does maturity predict whether an arrangement is stated at all," answer: modestly, yes) and both readings remain legitimate. "Does geography predict whether an arrangement is stated at all" remains confirmed at conventional thresholds for APAC (above) — and the dashboard panel shows the maturity views.
...
**Caveat on missingness:** 37% of the analytical cohort states no work arrangement at all, and that rate is not uniform by region — APAC's 49% not-stated rate (above), combined with its distinct stated-arrangement mix (more onsite, less hybrid), remains a confirmed, not merely directional, difference at this n, and this revision's tighter significance (§4.9 note above) suggests the pattern is not an artefact of a particular corpus snapshot. Whether it reflects different posting conventions (many APAC postings were sourced via LinkedIn/company career pages that omit a work-arrangement field entirely, or via channels more likely to post explicitly onsite roles) or genuine underlying differences in how APAC employers set policy is not resolvable from JD text alone.

---

### Finding I: With `ai_role`, `testing_framing`, and `loss_aversion_framing` coded on the full cohort (n=478; §9.3), a systematic sweep against every other categorical dimension and tool flag surfaces several relationships, all stable at the current n

**Testing accountability tracks the fear register closely (χ²=113.14, p<0.0001, V=0.34, n=478):**

| testing_framing | high | moderate | none |
|---|---|---|---|
| absent (n=117) | 9% | 36% | 56% |
| responsibility (n=289) | 27% | 65% | 8% |
| tool_listed (n=72) | 11% | 64% | 25% |
...
|---|---|---|---|---|
| velocity_vs_rigour × domain_risk (Prediction 1) | χ² | <0.0001 | 0.19 | Stable at n=440 (was p=0.0002, V=0.17 at n=389) — small real effect, high-risk roles more rigour-dominant |
| velocity_vs_rigour × has_dbt (Prediction 1 comparator) | χ² | 0.047 | 0.12 | **Newly significant at n=427 AE/BI** (was p=0.14, V=0.10 at n=389) — a borderline effect crossing a borderline line after several snapshots hovering just below it; see §4.0 |
| ai_role × stakeholder_orientation (Prediction 2) | χ² | 0.133 | 0.11 | Still not significant at n=478 (was p=0.28 at n=440) — stable non-result |
...
| domain_risk × greenfield_vs_fix | χ² | 0.184 | 0.08 | **Still not significant at n=478** (was p=0.304, V=0.07 at n=440) — this relationship has crossed the threshold in both directions across earlier snapshots and remains null; read as a marginal effect this corpus size can't reliably detect either way, not a reversal (§4.9 Finding B) |
| jd_authorship × has_dbt | χ² | <0.0001 | 0.24 | Hiring-manager JDs name dbt ~1.8× more than recruiter JDs — stable |
| geo_region (APAC vs. rest) × jd_authorship | χ² | 0.014 | 0.11 | **Now significant again at n=478** (was p=0.071, V=0.11 at n=440) — direction unchanged (87% vs. 72% hiring-manager-authored) and effect size unchanged; the crossing reflects the non-APAC comparison group shifting, not the APAC cell itself; Finding G, §9.5 |
| geo_region (APAC vs. rest) × work_arrangement (4-category) | χ² | 0.0014 | 0.18 | **Remains significant at n=478, effect size strengthened** (was p=0.012, V=0.16 at n=440) — APAC's not-stated rate (49% vs. 35%) and its stated-arrangement mix (more onsite, less hybrid) both continue to measurably differ from the rest of the corpus; Finding H, §9.5 |
| seniority × autonomy_level | χ² | <0.0001 | 0.33 | Significant overall; "Senior" (n=183, still not the modal title by count — "Mid" is, at n=242) still predicts strongly (48% strategic) |
| stakeholder_orientation × autonomy_level | χ² | 0.060 | 0.13 | **Just outside p<0.05 on the significant side at n=478** (was p=0.175, V=0.11 at n=440) — this relationship has crossed the threshold multiple times across snapshots; read as a stable, small, directionally-consistent effect this corpus size is only now approaching the power to confirm (§4.9 Finding E) |
...
| collaboration_width × data_team_maturity | — | — | — | Still does not support a claim at n=478 (§4.7) |
| work_arrangement × geo_region (stated subset) | χ² | <0.0001 | 0.50 | Strongest association found, but unreliable — most cells <5 (Finding H) |
| language_gate_type × geo_region (3-way: Europe/APAC/NYC metro) | χ² | <0.0001 | 0.22 | **Largest region effect in this document.** Hard/soft language gates: Europe 40.0%, APAC 7.2%, NYC metro 0% — near-exclusively a European phenomenon; §9.13, n=428 |
| velocity_vs_rigour × geo_region (3-way: Europe/APAC/NYC metro) | χ² | 0.0008 | 0.15 | `rigour` share steps down Europe 72.1% → APAC 63.8% → NYC 44.8% with domain_risk composition held flat across regions — a framing/dialect gap, not a risk gap; §9.13, n=428 |
| work_arrangement × data_team_maturity (stated subset) | χ² | 0.0006 | 0.18 | **Still significant at n=478** (was p=0.0002, V=0.20 at n=440) — same direction as every prior snapshot (mature teams skew hybrid) (Finding H) |
| work_arrangement × autonomy_level (stated subset) | χ² | 0.008 | 0.15 | **Still marginally significant** (was p=0.022, V=0.14 at n=440) — mixed-autonomy roles remain the most hybrid-concentrated, strategic roles show the highest onsite and remote shares, effect sits right at the small-effect floor (Finding H) |
| work_arrangement × everything else (tool stack, seniority, rigour, domain risk) | χ² | >0.20 | ≤0.13 | Null — unrelated to arrangement |
| loss_aversion_framing × domain_risk | χ² | <0.0001 | 0.42 | 76% of high-loss-aversion JDs are high-domain-risk (equivalently: 61% of high-domain-risk roles carry high loss-aversion framing) (Finding I) |
...
| loss_aversion_framing × velocity_vs_rigour | χ² | <0.0001 | 0.38 | Cleaner than domain_risk's own link to rigour — 98% of high-loss-aversion JDs are rigour-framed (Finding I) |
| testing_framing × has_dbt | χ² | <0.0001 | 0.36 | dbt JDs 78% likely to frame testing as owned responsibility vs. 38% without dbt (Finding I) |
...
| ai_role × autonomy_level | χ² | <0.0001 | 0.22 | Unexpected direction, stable: `ai_user` and `ai_enabler` roles are now equally strategic-leaning (49% each), both well ahead of `none` (23%) (Finding I) |
...
`ai_role` classifies whether the JD expects the candidate to *use* AI tools, *build* infrastructure AI systems consume, or neither. **Coded on the full analytical cohort (n=478)** — a bug in `scripts/write_jd.py` had silently dropped this field (and `testing_framing`, `loss_aversion_framing`) from JSON output for a long stretch of the corpus even when correctly classified; the backlog was fully re-coded against the JD archive text and the codebook (§9.3).
...
This is Prediction 2 from §4.0. **65% of JDs expect no AI skill from the candidate**, unchanged from n=440, against the dbt 2026 report's claim of 72% *daily* AI coding use among survey respondents. The gap between claimed personal-workflow adoption and formal hiring criteria has held steady. χ² for `ai_role` × `stakeholder_orientation` (n=478) remains non-significant (p=0.13, V=0.11; §4.0) — the `ai_enabler` cohort still leans toward `internal_data` and `mixed` stakeholder orientation, and `ai_user` leans similarly, and the association's weakness is stable rather than trending toward zero.

**Actionable read:** `ai_enabler` roles → demonstrate data infrastructure built specifically for AI consumption. `ai_user` roles → demonstrate fluency with AI coding tools directly (Copilot, Claude Code, Cursor) as a nontrivial minority expectation. `none` (still the majority at 65%) → AI tool fluency is not a stated differentiator; leading with it misreads what's being screened for. This batch (§9.12) contributed three clean examples of each non-`none` category: `ai_user` (Butterfly Network, MUBI, Zego — all naming specific AI coding tools like Claude Code) and `ai_enabler` (Retail Insight, foodpanda, N-able — all building AI-consumable semantic layers or agentic infrastructure).

`ai_role` continues to track `autonomy_level` (χ²=45.58, p<0.0001, V=0.22) and `greenfield_vs_fix` (χ²=30.28, p<0.0001, V=0.18) at n=478 — see Finding I (§4.9) for the counter-intuitive direction (`ai_user` and `ai_enabler` roles skew *more* strategic and *more* greenfield, not less, and have now converged to near-identical strategic shares).

---

### 4.11 Testing framing: governance accountability is a majority hiring criterion

`testing_framing` distinguishes whether testing/data quality appears as something the candidate *owns*, a listed tool, or absent. **Coded on the full analytical cohort (n=478)** — see §9.3 for the write-pipeline bug that delayed this.

| testing_framing | n | % (n=478) |
|-----------------|---|---|
| responsibility | 289 | 60% |
| absent | 117 | 24% |
| tool_listed | 72 | 15% |

**60% of JDs frame testing as an owned responsibility** — action verbs (own, ensure, define, implement) paired with quality/data-contracts/observability language, essentially unchanged from 61% at n=440. This is the clearest confirmation in the dataset of dbt 2026's "trust gap" narrative at the level of formal hiring criteria, distinct from §4.1's rigour finding: two rigour-coded JDs can differ in whether the *individual hire* is personally accountable for quality or whether it's team culture. `testing_framing = responsibility` identifies the former. `testing_framing × velocity_vs_rigour` is significant (χ²=66.71, p<0.0001, V=0.26, n=478): `responsibility`-coded JDs are 81% rigour-framed vs. 44% for `absent`-coded JDs — testing ownership and rigour framing move together but are not the same signal, since a substantial share of the cohort is rigour-framed with no testing-ownership language at all.
...
`testing_framing`'s strongest tool-stack link (`has_dbt`, χ²=59.42, p<0.0001, V=0.36) and its link to `jd_authorship` (χ²=53.96, p<0.0001, V=0.24) and `loss_aversion_framing` (χ²=113.14, p<0.0001, V=0.34) all hold stable at n=478 — see Finding I (§4.9) for detail. APAC roles run somewhat above the corpus average on `responsibility` framing, consistent with prior snapshots; §9.5.

---

### 4.12 Loss-aversion framing: the market fears operational failure, not AI hallucinations

`loss_aversion_framing` classifies what the JD is afraid of: nothing, operational failure (outages, SLOs), or compliance/stakeholder-trust failure. **Coded on the full analytical cohort (n=478)** — see §9.3.

| loss_aversion_framing | n | % (n=478) |
|-----------------------|---|---|
| moderate | 276 | 58% |
| none | 107 | 22% |
| high | 95 | 20% |

Roughly four in five JDs carry some fear signal, but it's still predominantly operational (58%), not the compliance/AI-trust framing the dbt 2026 report leads with (71% citing fear of hallucinated outputs). `high` loss-aversion framing holds at 20%, unchanged from n=440. `loss_aversion_framing × domain_risk` is the strongest relationship among these three dimensions (χ²=166.01, p<0.0001, V=0.42, n=478, essentially unchanged from n=440): of JDs with `high` loss-aversion framing, 76% are `high`-domain-risk, vs. 17% of `moderate`-risk and 0% of `low`-risk roles carrying `high` framing — the fear register tracks real domain stakes closely, which is reassuring for the codebook's construct validity on this dimension. `high` loss-aversion framing remains concentrated in finance-adjacent and regulated-sector roles — this batch's fintech/payments additions (Aperia Solutions, Current, payabl., CMC Markets) all coded `high`, consistent with the pattern; APAC's own `high` rate remains below the corpus average, consistent with prior readings (§9.5).
...
1. **Mentorship & Leadership × `autonomy_level` (χ²=23.73, p<0.0001, V=0.23, n=443) — the one relationship in this section that received an actual confounder check, and passed it.** Mentorship/leadership language climbs from 6% of execution-coded JDs to 12% mixed to 26% strategic. Because `autonomy_level` and seniority title are themselves correlated (§4.5), this could just be seniority in disguise — so it was re-tested within seniority strata specifically: **within "Mid" titles alone**, the gradient is 2%→10%→8% (execution→mixed→strategic, noisy at these cell sizes); **within "Senior" titles alone**, it's 13%→14%→23%. The gradient is directionally consistent within the senior stratum specifically; the mid stratum is noisier at this n, so read the within-seniority claim as suggestive rather than as clean as the unstratified figure implies. This is essentially unchanged from the n=423 reading (χ²=20.98, V=0.22).
2. **Data Infrastructure & Warehouse Ops × `jd_authorship` (χ²=21.79, p<0.0001, V=0.22, n=443) — clean-screen only, not independently confounder-checked.** Hiring-manager-authored JDs name warehouse/infrastructure responsibilities at 56% vs. 13% for recruiter-authored — a considerably wider gap than authorship's already-known link to whether dbt is merely named (§4.6, §4.9 Finding G). Directionally consistent with the revealed-preference logic elsewhere in this document (naming a platform's actual cost/governance responsibilities requires knowing the team's real infrastructure problem, not just its tool list), but this specific pairing has not been re-tested against a plausible confounder the way (1) was. Essentially unchanged from the n=423 reading (χ²=22.54, V=0.23).
...
**A relationship that looked real and didn't survive scrutiny — kept as a worked example, not dropped:** `Architecture & Platform Strategy × work_arrangement` at one point cleared the p<0.01 screen and on its own read as a headline — "remote roles carry less architectural scope." A taxonomy audit corrected several loose keywords in this theme, and the pairing **still does not clear the screen at current corpus size, and the gap has widened further** (χ²=5.64, p=0.13, V=0.11, n=443; overall: hybrid 52%, remote 44%, onsite 33%, not_stated 43% — was p=0.06, V=0.13 at n=423) — it was never a stratification failure story to begin with; it was a keyword-precision artifact of the looser pre-audit pattern, and it remains a null after the fix, drifting further from significance rather than back toward it. The stratification breakdown is kept below as a worked example of *why* a stratification check matters, but the more direct lesson from this specific relationship turned out to be about pattern precision, not confounding:

- Within `data_team_maturity=early`: hybrid 41% (n=29); remote 30% (n=10); onsite 36% (n=11); not_stated 52% (n=21)
- Within `data_team_maturity=mid`: hybrid 49% (n=124); remote 48% (n=29); onsite 27% (n=11); not_stated 38% (n=95)
- Within `data_team_maturity=mature`: hybrid 65% (n=55); remote 50% (n=4); onsite 40% (n=5); not_stated 49% (n=49)
...
Full theme definitions, all 304 tested pairs, the complete construct-overlap table, and this same write-up regenerated fresh on every corpus update live in `analysis/responsibility_taxonomy.md` (`python3 analysis/responsibility_taxonomy.py` to reproduce).

---

## 5. What the survey claims vs. what JDs show

| dbt 2026 claim | JD evidence (n=478 analytical cohort) | Assessment |
|----------------|-------------|------------|
| 83% prioritise data trust | 68% rigour-oriented; testing framing coded on full cohort (n=478, 60% responsibility framing) | Confirmed at the orientation level and at the testing-accountability level; rigour share holds flat against n=440 (68%→68%, §4.1) |
| 72% use AI in coding workflows daily | 65% of JDs (n=478, full coverage) expect no AI skill; 15% name AI coding tools directly (`ai_user`) | Gap persists, essentially unchanged from the n=440 reading (65%/14%) — Prediction 2 (§4.0), first half still holds, second half (structural concentration) remains non-significant |
| AI adoption outpacing governance (72% vs. 24%) | Governance accountability (60% of n=478 full cohort); AI hiring signal 36% (`ai_enabler`+`ai_user`) | The JD evidence still suggests governance accountability further institutionalised than AI hiring criteria, and the ratio has held stable |
| Fear of hallucinated outputs (71%) | `loss_aversion_framing = high` is 20% of n=478 full cohort; 58% report operational reliability concerns | Not confirmed — dominant fear is still operational reliability, not AI-trust hallucination; both figures essentially unchanged from the n=440 reading |
| Rigour framing tracks risk/stakes | χ²=30.20, p<0.0001, V=0.18 (§4.0/§4.2, Prediction 1) — stable at n=462 AE/BI | Confirmed for `domain_risk`; the `has_dbt` comparator that previously ran alongside it (§4.0) also remains significant at a comparable effect size (p=0.015, V=0.13) — rigour tracks risk somewhat more strongly than tool adoption, but the two are no longer cleanly separated by significance |
| dbt is the field standard | 65% of AE/BI JDs mention dbt (n=462) | Real but not universal; roughly one in three AE/BI roles run dbt-free stacks; stable across twelve consecutive snapshots, including in the APAC subset specifically (§9.5) |

**The governance-vs-AI gap inverts the dbt narrative's emphasis**, though both halves are visible in the data: dbt 2026 frames the central tension as AI adoption outrunning governance readiness. The JD evidence shows governance accountability further along toward institutionalisation (60% of coded roles) than AI hiring criteria (36% combined `ai_enabler`+`ai_user`). Whether that reflects genuine institutional maturity in analytics engineering specifically, or simply that governance is an older, more diffused fashion than AI-assisted coding, the data doesn't resolve — but the dbt framing of governance as the deficit side of the gap is not what employer hiring language shows.
...
**Current state: all three dimensions are coded on the full analytical cohort (n=478, including all JDs added since the fix, surviving both 2026-07-25 dedup passes, and every subsequent expansion through 2026-08-13, §9.9–§9.12)**, with consistent `evidence.{dim}` (quote) + `evidence.{dim}_explanation` (reasoning) entries on every record, and no legacy-format duplication. Findings in §4.10–4.12 and Prediction 2 (§4.0) are stated against the full current n, not a small coded subset — this changed several conclusions materially when the fix first landed at n=272 (§4.0, §4.10), and the corpus has grown, been deduplicated twice, and grown again multiple times since without disturbing that fix.

### 9.4 What n=478 supports

At n=478, the margin of error on a single proportion is approximately ±4.1pp at 95% confidence (Wilson interval) — the 68% rigour finding (§4.1) is defensible as "likely between 64% and 72%," not as a precise market figure. Cross-tabs with cell sizes below ~15 (junior seniority, pure velocity, low domain-risk in some cross-tabs) are illustrative, not evidential, and are flagged as such at each occurrence above. The corpus additions from the n=123 baseline through n=478 provided meaningful confidence-interval tightening and, along the way, flipped several relationships across the significance threshold in both directions (§4.0, §4.9 document each one as it happened). The most recent +18-record expansion (§9.12) held every headline distribution essentially flat — rigour held at 68% (unchanged from n=440, §4.1), while `has_dbt` (65%, up ~0.6pp) and `ai_role=none` (65%, unchanged) also stayed within the range prior batches have produced — see §9.12 for the batch's own composition, notably skewed toward larger established employers rather than the consumer/gaming mix that had driven the prior rigour decline. The relationships that tested null at n=440 — `domain_risk × greenfield_vs_fix`, `ai_role × stakeholder_orientation` — remain null at n=478; `stakeholder_orientation × autonomy_level`, also null at n=440, now sits just outside p<0.05 on the significant side (p=0.060, up from p=0.175) — not yet confirmed, but worth watching at the next update. `velocity_vs_rigour × has_dbt`, which first crossed p<0.05 at n=427, remains significant at n=462 with a wider margin (p=0.015, V=0.13, up from p=0.047, V=0.12) — a small effect that has now held across two consecutive corpus snapshots. Pattern stability continues to hold for the strongest relationships (maturity × mission, domain_risk × stakeholder, domain_risk × rigour) across every snapshot from n=123 to n=478.

### 9.5 What the geographic concentration means, and what the APAC stratum shows

This remains a primarily European, Berlin-heavy dataset. The APAC stratum, built by a deliberate scraping pass in late July and reinforced by subsequent batches (§9.8, §9.9), holds at **69 roles (14% of the analytical cohort)** — up from 55 at the last snapshot, and remains the largest single geographic bucket in the corpus, ahead of UK/remote — still large enough to run a direct APAC-vs-rest-of-corpus comparison rather than only disclaiming the gap, as earlier snapshots of this document had to.

**Most substantive dimensions still track closely, but rigour orientation and work-arrangement disclosure both continue to differ from the rest of the corpus, the latter confirmed rather than directional.** Domain risk (67% moderate vs. 68%), data team maturity (51% mid vs. 59%), dbt prevalence (66% vs. 65%), and `testing_framing` mix (62% responsibility vs. 60%) all sit within a normal range of the non-APAC corpus. Rigour orientation remains lower for APAC (64%) than the rest of the corpus (69%) — consistent in direction with prior snapshots, though still not formally tested given the stratum's size. `loss_aversion_framing = high` also remains lower for APAC (17% vs. 20% for the rest of the corpus), unchanged in direction from prior snapshots. None of the risk/maturity/dbt/testing comparisons are statistically distinguishable at this n, but the rigour and loss-aversion gap having now held in the same direction across multiple, partly-independent scraping batches makes it a steadier signal than a single-snapshot reading would be.

**Two dimensions that were directional at earlier snapshots remain significant:**

| Dimension | APAC (n=69) | Rest of corpus (n=409) | Test |
|---|---|---|---|
| `jd_authorship = hiring_manager` | 87% | 72% | χ²=6.04, p=0.014, V=0.11 |
| `work_arrangement` (full 4-category: hybrid/not_stated/onsite/remote) | 35% / 49% / 13% / 3% | 50% / 35% / 5% / 10% | χ²=15.51, p=0.0014, V=0.18 |

Held carefully: `jd_authorship`'s LLM self-consistency is the lowest of any dimension in the codebook (0.58, §3), so part of this gap could be a codebook-boundary artefact interacting with how APAC postings happen to be formatted (many sourced via LinkedIn/company career pages with detailed technical bullet lists, which the heuristic may read as "hiring-manager-authored" regardless of who actually wrote them) rather than a real difference in who authors these JDs — and the test has crossed the conventional threshold in both directions across earlier snapshots, which is itself a reason for caution. The work-arrangement picture continues to differ in kind, not just degree: APAC's not-stated rate (49% vs. 35%) is the same direction as every prior snapshot, and APAC continues to carry a distinctly higher onsite share (13% vs. 5%) among stated arrangements, with the hybrid gap holding at roughly the same margin as the last snapshot (35% vs. 50%). Among APAC roles that do state an arrangement, hybrid still dominates numerically but the onsite share stands out against the rest of the corpus.
...
§9.5 tests APAC against "the rest of the corpus" — a framing that was reasonable when APAC was the only non-European stratum large enough to test, but which folds the (much smaller, but now large-enough) `nyc_metro` cluster (n=29, §9.12) into the European comparison group by default. Once NYC is split out as its own arm, three regions are jointly comparable: **Europe (n=330), APAC (n=69), NYC metro (n=29)** — 428 of the 478-record analytical cohort with a usable macro-region (`global_remote` and `other` excluded as incoherent geographies). A chi-square sweep of all thirteen categorical Layer B dimensions against this 3-way split found six that clear p<0.05: `velocity_vs_rigour` (p=0.0008, V=0.15), `data_team_maturity` (p=0.020, V=0.12), `jd_authorship` (p=0.0095, V=0.13), `stakeholder_orientation` (p=0.021, V=0.15), `work_arrangement` (p=0.0016, V=0.16), and `language_gate_type` (p<0.0001, **V=0.22 — the largest effect size of any relationship tested against region in this document**, larger than either of §9.5's headline findings). `domain_risk`, `autonomy_level`, `ai_role`, `testing_framing`, and `loss_aversion_framing` show no significant regional difference — worth stating positively, not just as an absence: a job seeker's read on genuine ownership, risk structure, and AI expectations should generalise across these three regions in this dataset; their read on rigour-language, language gates, and salary disclosure should not.

**Two confounds were checked before trusting this.** First, `domain_risk` composition is flat across all three regions (moderate 65–70%, high 24–30%, low 1–9% everywhere) — the effects below aren't a byproduct of NYC or APAC happening to sample riskier or safer companies. Second, the NYC cluster is not one loud employer or one ATS's house style: 29 roles span 27 distinct companies (only Current and DoorDash repeat), and its ATS mix (Greenhouse 11, Ashby 5, LinkedIn 5, Workday 3, unknown 5) doesn't concentrate the way a single-source artifact would. n=29 is still thin, so NYC percentages here should be read as directional, not precise — but they are not obviously an artifact of collection method.

**`language_gate_type` is the standout finding this section adds.** Hard language requirements (fluency/C1-C2 gates): Europe 29.7%, APAC 5.8%, NYC 0%. Adding soft gates, Europe reaches 40.0% versus APAC's 7.2% and NYC's 0%. This is almost certainly a genuine market feature — client-facing analytics roles across DACH/Benelux/France routinely gate on the local language, with no real equivalent in an English-default APAC or US hiring market — and it is the single most actionable "know before you apply" fact a non-European candidate in this corpus could act on. **This dimension has never previously been tested against `geo_region` anywhere in this document**, and it isn't surfaced in the seeker-mode hygiene card in `index.html` either (that card reports a single blended `hardLangPct` across all regions, currently ~useless for a candidate targeting a specific market — see the cross-reference note below).

**`velocity_vs_rigour` drops step-wise Europe → APAC → NYC, and — because domain-risk composition is flat — this is a framing/dialect difference, not a risk difference.** `rigour`-coded: Europe 72.1%, APAC 63.8%, NYC 44.8%. Reading the 29 NYC records by hand: the velocity-coded roles cluster in genuinely earlier-stage consumer fintech/proptech (Copilot Money, CurbWaste, Profound, Spot & Tango), while the rigour-coded ones are the regulated-finance names (Current, Gemini, New York Life, Neuberger Berman) — so this isn't "NYC roles are less rigorous," it's that NYC JDs are more willing to name velocity plainly when that's the honest framing for an early-stage company, whereas the European corpus defaults to rigour-coded vocabulary even for comparable-risk, non-regulated work. Practically: a `rigour` classification from a European JD is weaker evidence of genuinely elevated stakes than the same classification from a US JD, because the base rate of reaching for that vocabulary differs by region independent of risk. (This calibration note has been added directly to `.claude/skills/classify-jd/SKILL.md`'s `velocity_vs_rigour` section so future classification is aware of the dialect effect without trying to correct for it at classification time.)

**`jd_authorship` and `work_arrangement` replicate and sharpen §9.5's APAC findings once NYC is added as a third point of comparison, but the likely driver is company profile, not region per se.** `hiring_manager`-authored: Europe 69.7%, APAC 87.0%, NYC 89.7%; the `recruiter`-boilerplate tail nearly vanishes outside Europe (8.5% → 2.9% → 0%). Both the APAC and NYC batches skew toward either large established single-market employers (Google, DoorDash, Fox Corporation, Siemens, Warner Bros. Discovery) or well-funded technically sophisticated startups (Gemini, Rogo, MoonPay) — segments where the req owner is also the JD author — while the European sample carries a longer mid-market/agency tail. Treat this as a company-size/maturity effect that correlates with region in this corpus's specific sampling, not a claim that APAC or NYC hiring managers inherently write better JDs. `work_arrangement`: APAC is both the most silent (49.3% `not_stated` vs. Europe 34.8%, NYC 34.5%) and, where stated, the most onsite-leaning (13.0% onsite vs. Europe 4.5%); NYC splits close to Europe on hybrid (48.3% vs. 54.8%) but shows a genuine zero on fully-remote across all 29 roles, worth flagging even at this n. The seeker-mode hygiene card's current blanket advice ("don't read silence as onsite by default, ask directly") is better calibrated for Europe than for APAC, where silence does correlate with a real onsite lean.

**`stakeholder_orientation`'s regional shift is the one finding here I'd flag as directional rather than load-bearing.** `mixed` orientation rises sharply outside Europe (Europe 15.5% → APAC 27.5% → NYC 41.4%), with `internal_data`-primary framing falling correspondingly (52.7% → 43.5% → 24.1%). This may be a genuine structural difference in how the analytics function sits in APAC/NYC organisations, or it may partly be a classifier mechanical effect — `mixed` is the catch-all when a JD names two functions with genuinely equal weight, and the same larger/more-mature companies driving the authorship finding above may simply name more stakeholder groups by virtue of size, independent of any real orientation shift. Not yet confirmed as substantive; worth re-checking once NYC's n grows.

**Salary disclosure: the sharpest single number in this section, and it should not be read as a market-culture finding.** Salary stated: Europe 20.3% (67/330), APAC 2.9% (2/69), NYC metro 82.8% (24/29). The NYC figure is very likely a **legal-regime effect**, not an employer-culture one — New York's pay transparency law requires a posted range, and the number reflects that law doing exactly what it was designed to do, not that NYC employers are more forthcoming by disposition. `index.html`'s current seeker hygiene card blends all regions into one `salaryPct` figure — that single blended number actively misleads in both directions: a US job seeker outside a pay-transparency jurisdiction would over-trust the disclosure norm, and an APAC job seeker (2.9%!) would be right to essentially never expect a stated range regardless of company quality.
